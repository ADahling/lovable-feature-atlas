...
| 2026-07-20 | Paper Cosmos palette namespaced as `--pc-*` tokens; ConstellationView is hex-value-free | 33 unique raw hex values across the constellation were consolidated into a documented `--pc-*` namespace in `styles.css` (page/surface/ink/emerald/gold/text/line families). Visual identical, single source of truth for the immersive map, main brand palette untouched. Last non-gold focus ring (`focus-within:ring-[…]/15` on the search input) normalized to `ring-gold/70`. |
