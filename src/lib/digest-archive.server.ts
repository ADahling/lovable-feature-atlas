...
export async function listPublishedDigestsImpl(): Promise<ArchiveListItem[]> {
  try {
    ...
    console.warn("[digest-archive] list unavailable; serving an empty archive", error);
    return [];
  }
}
...
export async function getPublishedDigestImpl(id: string): Promise<ArchiveDetail | null> {
  try {
    const { data: row, error } = await supabaseAdmin
      .from("digest_send_log")
      .select(
        "id,sent_at,subject,feature_count,period_start,period_end,catalogued_total,shipped_feature_ids,catalogued_feature_ids,trigger,status",
      )
      .eq("id", id)
      .neq("trigger", "preview")
      .in("status", ["ok", "partial"])
    ...
    const shipped = shippedIds
      ...
      .filter((feature): feature is ArchiveFeature => Boolean(feature));
    const catalogued = cataloguedIds
      .map((featureId) => byId.get(featureId))
      .filter((feature): feature is ArchiveFeature => Boolean(feature));
    ...
    console.warn(`[digest-archive] ${id} unavailable; treating it as unpublished`, error);
    return null;
  }
}
