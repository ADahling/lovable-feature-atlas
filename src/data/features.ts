// Lovable Features Dataset — migrated from legacy public/atlas/features.js
// Every record preserved verbatim. Recent (May 2026 inbox) additions appended below.

export interface Feature {
  id: string;
  name: string;
  category: string;
  status: "GA" | "Beta" | "Removed";
  releaseDate: string;
  pricing: string;
  icon: string;
  tagline: string;
  description: string;
  capabilities: string[];
  useCases: string[];
  source: string;
}

export const features: Feature[] = [
  {
    id: "agent-mode",
    name: "Agent Mode",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2025-02-11",
    pricing: "All plans",
    icon: "🤖",
    tagline: "Autonomous, end-to-end execution mode.",
    description:
      "Agent Mode takes ownership of a task end-to-end — understands intent, explores the codebase, applies changes across files, resolves issues mid-implementation, and verifies results. All changes surface as file diffs and a step-by-step Details view.",
    capabilities: [
      "Implement features once an approach is decided",
      "Fix bugs end-to-end across frontend, backend, and config",
      "Refactor across multiple files",
      "Inspect logs and network activity to identify problems",
      "Fetch external docs or assets, generate or edit images and video",
      "Verify results before finishing",
    ],
    useCases: [
      "Ship a complete CRUD feature from a single prompt",
      "Diagnose and fix a flaky auth flow",
      "Refactor a component library to a new design system",
    ],
    source: "https://docs.lovable.dev/features/agent-mode",
  },
  {
    id: "plan-mode",
    name: "Plan Mode",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "1 credit per message",
    icon: "🗺️",
    tagline: "Brainstorm and design before any code is written.",
    description:
      "Plan Mode is Lovable's planning and reasoning mode. It reasons across multiple steps, inspects files/logs/context, and proposes a formal plan you can inspect, edit, and approve. Plan Mode never modifies code — the latest approved plan is saved to .lovable/plan.md and earlier plans remain in chat history.",
    capabilities: [
      "Multi-step reasoning across files and logs",
      "Clarifying questions before proposing changes",
      "Editable, markdown-rendered plans",
      "Approve to hand off to Agent Mode",
      "Plans persisted to .lovable/plan.md",
    ],
    useCases: [
      "Scope a new feature before spending build credits",
      "Investigate a tricky bug and decide on a fix",
      "Architect a database schema with the agent as a thought partner",
    ],
    source: "https://docs.lovable.dev/features/plan-mode",
  },
  {
    id: "prompt-queue",
    name: "Prompt Queue",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "All plans",
    icon: "📥",
    tagline: "Queue, reorder, and repeat prompts while the agent works.",
    description:
      "Queue prompts while Lovable is mid-task. The agent processes one at a time, and you can pause, resume, reorder, edit, or repeat a prompt up to 50× — useful for batching small follow-ups or chaining smart suggestions.",
    capabilities: [
      "One-at-a-time sequential execution",
      "Reorder, edit, pause, or resume queued prompts",
      "Repeat a prompt up to 50× (great for fanned-out generation)",
      "Queue smart suggestions automatically surfaced by Lovable",
    ],
    useCases: [
      "Batch a series of UI tweaks while the agent is in a long build",
      "Generate 20 variations of a marketing component",
      "Walk away — return to a completed multi-step sprint",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "smarter-agent",
    name: "Smarter Agent (15-min sessions)",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "All plans",
    icon: "⏱️",
    tagline: "Processing extended to 15 minutes per request.",
    description:
      "The Lovable agent's per-request processing time was extended to 15 minutes, supporting longer browser-testing sessions and more complex, multi-file tasks. The agent can also generate videos, suggest publishing, clean up unused edge functions, and understand TypeScript projects at IDE level.",
    capabilities: [
      "Up to 15-minute single-request runs",
      "Video generation in-thread",
      "Edge-function hygiene (cleans up unused functions)",
      "IDE-level TypeScript awareness",
    ],
    useCases: [
      "Long browser-testing flows that span many pages",
      "Big refactors that span dozens of files",
      "Generate a promo video as part of a build",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "code-execution",
    name: "Code Execution in Chat",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Enterprise",
    icon: "▶️",
    tagline: "Run code and analyze data without modifying your project.",
    description:
      "On Enterprise plans, Lovable can analyze data, generate files, and execute code directly in chat without touching project source. Pair with File Generation to produce PDF/Excel/PPT artifacts on demand.",
    capabilities: [
      "Sandboxed code execution in-thread",
      "Analyze attached datasets",
      "Generate downloadable PDF, Excel, PPT files",
      "No project source modification",
    ],
    useCases: [
      "Ad-hoc data analysis on a sales CSV",
      "Produce a one-off report PDF for a client",
      "Prototype a script without committing it",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "visual-edits",
    name: "Visual Edits",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-02-12",
    pricing: "Free (within daily limits)",
    icon: "🎨",
    tagline: "Figma-like, Tailwind-native direct manipulation.",
    description:
      "Select any element in the live preview and edit text, colors, fonts, spacing, layout, and images directly — no prompts required. Free, fast, and credit-friendly with undo always available.",
    capabilities: [
      "Click to select any element (multi-select with Shift)",
      "Edit text, colors, fonts, line-height, weight",
      "Layout, alignment, per-side margins/padding",
      "Replace images by upload or URL; generate AI images",
      "Tailwind-native output — no design drift",
    ],
    useCases: [
      "Tighten spacing on a hero section",
      "Swap a placeholder image for a generated one",
      "Designers fix copy and color without burning credits",
    ],
    source: "https://docs.lovable.dev/features/design",
  },
  {
    id: "ai-visual-edits",
    name: "AI Visual Edits",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Free (within daily limits)",
    icon: "✨",
    tagline: "Visual edits that propagate across dynamic data.",
    description:
      "AI-powered Visual Edits now update UI elements across the entire app, including dynamic content rendered from databases and APIs. Includes zoom and crop tools for inspecting specific image regions.",
    capabilities: [
      "Edit a card style once → applies everywhere the component renders",
      "Works against dynamic data from Supabase/Lovable Cloud",
      "Image zoom and crop inspection tools",
    ],
    useCases: [
      "Rebrand a product card list without rewriting JSX",
      "Restyle every row in a data table consistently",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "code-mode",
    name: "Code Mode",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-05-07",
    pricing: "Paid plans",
    icon: "💻",
    tagline: "Inspect and edit project code directly inside Lovable.",
    description:
      "View and edit your project's underlying codebase from inside Lovable — inspect files, make targeted edits, troubleshoot without exporting or switching tools. Available on paid plans.",
    capabilities: [
      "File tree browser with diff view",
      "In-place text editing with syntax highlighting",
      "Round-trip safe with Agent Mode",
    ],
    useCases: [
      "Tweak a single Tailwind class without a prompt",
      "Read a generated Supabase migration before approving",
    ],
    source: "https://docs.lovable.dev/features/code-mode",
  },
  {
    id: "chat-history-search",
    name: "Chat History Search",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "🔎",
    tagline: "Keyword and semantic search across full project history.",
    description:
      "Search and reference your full project conversation history with keyword search and semantic question answering. Find why you made a decision, what prompt produced a result, or jump back into an old thread.",
    capabilities: [
      "Keyword search across all conversations",
      "Semantic Q&A grounded on prior messages",
      "Reference prior threads in new prompts",
    ],
    useCases: [
      "Find the prompt that produced a specific component",
      "Audit decision history for a feature",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "project-comments",
    name: "Project Comments",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "💬",
    tagline: "Annotate the preview — @Lovable to trigger the agent.",
    description:
      "Leave inline annotations on the project preview. Use @Lovable or 'Send to chat' to convert a comment into an agent task.",
    capabilities: [
      "Pin comments to elements on the preview",
      "Mention @Lovable to trigger Agent Mode",
      "Collaborator-friendly review flow",
    ],
    useCases: [
      "PM review pass on a staging build",
      "Designer leaves spacing notes for the agent to action",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "cross-project-ref",
    name: "Cross-Project Referencing",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "🔗",
    tagline: "Reuse implementations from other projects via @ mentions.",
    description:
      "Reference and reuse components, schemas, or patterns from your other Lovable projects using @ mentions. The agent fetches the referenced context to inform the build.",
    capabilities: [
      "@-mention any project in your workspace",
      "Reuse components, schemas, and config",
      "Inherits design system across a portfolio",
    ],
    useCases: [
      "Carry a design system across multiple client builds",
      "Clone an auth flow you already perfected",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "github-sync",
    name: "GitHub Sync",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2024-12-01",
    pricing: "All plans",
    icon: "🐙",
    tagline: "Two-way GitHub sync with branch switching and co-author commits.",
    description:
      "Bi-directional sync between Lovable and GitHub. Branch switching is enabled by default. As of March 2026, commits include the user's GitHub identity as a co-author for clean attribution.",
    capabilities: [
      "Two-way sync (push/pull)",
      "Branch switching from the editor",
      "Co-author attribution on commits (Mar 2026)",
      "Pair with CI/CD or QA tech agents",
    ],
    useCases: [
      "Hand-off to a human dev team mid-project",
      "Wire up Vercel/Netlify deploy previews",
      "Run automated QA on every PR",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "supabase",
    name: "Supabase Integration",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2024-11-01",
    pricing: "All plans",
    icon: "🟩",
    tagline: "Auth, Postgres, storage, edge functions — wired in.",
    description:
      "Native Supabase integration backs Lovable Cloud and supports BYO Supabase projects. The agent can test authenticated edge functions, and resource exhaustion alerts now run hourly. Self-hosting and migration paths are documented.",
    capabilities: [
      "Auth (email, OAuth, magic link)",
      "Postgres with row-level security",
      "Storage buckets with management UI",
      "Edge functions — agent tests authenticated calls",
      "Hourly resource alerts",
    ],
    useCases: [
      "Full-stack SaaS MVPs with login",
      "Migrate from Lovable Cloud to managed/self-hosted Supabase",
    ],
    source: "https://docs.lovable.dev/tips-tricks/external-deployment-hosting",
  },
  {
    id: "stripe",
    name: "Stripe Payments",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Paid plans",
    icon: "💳",
    tagline: "Chat-driven Stripe (and Paddle) setup for subs and one-time.",
    description:
      "Lovable Payments adds subscriptions and one-time payments via Paddle or Stripe. The agent handles account creation, webhooks, and data. Stripe-only flow available since April 2026 via chat-driven auto-setup once Supabase + Stripe Secret Key are configured.",
    capabilities: [
      "Subscriptions and one-time payments",
      "Webhook handling generated automatically",
      "Paddle or Stripe provider choice",
      "Chat-driven, no manual Payment Links",
    ],
    useCases: [
      "Launch a paid SaaS with tiers in an afternoon",
      "Add a one-time digital purchase to a landing page",
    ],
    source: "https://docs.lovable.dev/integrations/stripe",
  },
  {
    id: "productivity-connectors",
    name: "Google Workspace + Microsoft 365",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "📧",
    tagline: "Gmail, Drive, Docs, Outlook, Teams, Word — wired up.",
    description:
      "Productivity Connectors give Lovable apps native access to Google Workspace (Gmail, Drive, Docs) and Microsoft 365 (Outlook, Teams, Word).",
    capabilities: [
      "Read/write to Gmail, Drive, Docs",
      "Send via Outlook, post to Teams",
      "Use for internal-tool builds",
    ],
    useCases: [
      "Build an internal CRM that drops notes into Docs",
      "Email-triggered workflows from your own inbox",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "data-connectors",
    name: "Data App Connectors",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "📊",
    tagline: "BigQuery, Databricks, Snowflake — SQL from chat.",
    description:
      "Connect to BigQuery, Databricks, and Snowflake for SQL queries and data workflows directly from the Lovable agent.",
    capabilities: [
      "SQL query from chat against warehouses",
      "Build dashboards on top of warehouse data",
      "Role-scoped access via OAuth",
    ],
    useCases: [
      "Internal analytics dashboard on Snowflake",
      "Quick ad-hoc data exploration",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "mcp-connectors",
    name: "MCP Chat Connectors",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans (custom MCP)",
    icon: "🔌",
    tagline: "Custom MCP servers + Hex, PostHog, Sentry, Sanity, Granola, Amplitude, Polar, Confidence, Fireflies, Gemini Enterprise, WordPress.",
    description:
      "Lovable supports Model Context Protocol chat connectors that bring external tools into the agent's context. Custom MCP servers are available on all paid plans (previously Business/Enterprise only).",
    capabilities: [
      "Custom MCP on all paid plans",
      "First-party MCPs: Hex, Confidence, PostHog, Sentry, Sanity, Polar, Granola, Amplitude, Fireflies, Gemini Enterprise, WordPress",
      "Chat-side context, no code changes required",
    ],
    useCases: [
      "Pull live Sentry errors into a debugging session",
      "Reference PostHog analytics while building",
      "Surface meeting notes from Fireflies/Granola",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "browser-testing",
    name: "Browser Testing",
    category: "Testing",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "All plans",
    icon: "🧪",
    tagline: "Real-browser interaction with screenshots and network logs.",
    description:
      "Browser Testing runs your app in a real, remote browser in a virtual environment. The agent can click buttons, fill forms, navigate pages, capture screenshots, read console + network logs, and detect runtime errors — at desktop, tablet, and mobile sizes.",
    capabilities: [
      "Real-browser clicks, inputs, navigation",
      "Screenshot capture and visual diffs",
      "Console + network log reads",
      "Runtime error detection",
      "Desktop / tablet / mobile viewports",
      "Runs against the project preview you're viewing",
    ],
    useCases: [
      "Verify a checkout flow after a refactor",
      "Confirm auth still works for a logged-in user",
      "Catch a mobile-only regression",
    ],
    source: "https://docs.lovable.dev/features/browser-testing",
  },
  {
    id: "browser-perf",
    name: "Browser Performance Tools",
    category: "Testing",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "⚡",
    tagline: "Profile Core Web Vitals, CPU, memory, and long tasks.",
    description:
      "Extends Browser Testing with performance profiling: Core Web Vitals (LCP/CLS/INP), CPU usage, long tasks, and memory.",
    capabilities: [
      "Core Web Vitals capture",
      "CPU and long-task profiling",
      "Memory inspection",
      "Per-route performance baselines",
    ],
    useCases: [
      "Catch a regression that tanks LCP",
      "Investigate a memory leak in a dashboard",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "test-live-env",
    name: "Test & Live Environments",
    category: "Testing",
    status: "Removed",
    releaseDate: "2026-02-05",
    pricing: "Free during beta",
    icon: "🌓",
    tagline: "Separate staging/prod databases (beta — sunset for new projects).",
    description:
      "Originally launched in Feb 2026 as a beta giving Lovable Cloud projects separate databases for staging and production. As of March 24, 2026 it's no longer available for new projects; existing projects retain access. Lovable plans to bring it back in an improved form.",
    capabilities: [
      "Separate dev/prod database environments",
      "Promote schema between environments",
      "Browser Testing ran against the test env",
    ],
    useCases: [
      "Safe schema migrations",
      "Stakeholder review on a staging URL",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "mobile-app",
    name: "Lovable Mobile App (iOS + Android)",
    category: "Mobile",
    status: "GA" as const,
    releaseDate: "2026-04-28",
    pricing: "All plans",
    icon: "📱",
    tagline: "Build, edit, and ship from your phone.",
    description:
      "Lovable launched its no-code AI app builder as a native mobile app on Apple's App Store and Google Play in late April 2026 — survived Apple's recent vibe-coding crackdown.",
    capabilities: [
      "Build and edit projects from iOS/Android",
      "Native mobile experience with sheets/menus",
      "Folders visible in mobile dashboard search (Apr 2026)",
    ],
    useCases: [
      "Sketch a build on the go and finish on desktop",
      "Approve a Plan Mode plan from your phone",
    ],
    source: "https://techcrunch.com/2026/04/28/lovable-launches-its-vibe-coding-app-on-ios-and-android/",
  },
  {
    id: "capacitor-expo",
    name: "Mobile App Export (Capacitor / Expo)",
    category: "Mobile",
    status: "GA" as const,
    releaseDate: "2025-08-01",
    pricing: "All plans",
    icon: "📦",
    tagline: "Wrap a Lovable web app as a native iOS/Android binary.",
    description:
      "Lovable projects can be wrapped as native mobile apps via Capacitor (most common) or Expo. The agent will install Capacitor CLI, initialize iOS/Android platforms, and configure the bundle ID.",
    capabilities: [
      "Capacitor + iOS/Android native shells",
      "Expo flow for React Native paths",
      "Reuses your existing Lovable UI",
    ],
    useCases: [
      "Ship a PWA-like app to the App Store",
      "Add push notifications via Capacitor plugins",
    ],
    source: "https://capgo.app/blog/transform-lovable-dev-app-to-mobile-with-capacitor/",
  },
  {
    id: "collaboration",
    name: "Real-Time Collaboration",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-05-05",
    pricing: "All plans",
    icon: "👥",
    tagline: "Invite collaborators by email — real-time co-editing.",
    description:
      "Press Share in the project editor and invite collaborators by email. Lovable supports real-time multi-user editing with live presence. Folder-level collaborator scoping added April 2026.",
    capabilities: [
      "Email-based invites",
      "Real-time presence and co-editing",
      "Folder-level collaborator scoping",
    ],
    useCases: [
      "Pair-build with a teammate",
      "Async design + dev handoffs",
    ],
    source: "https://docs.lovable.dev/features/collaboration",
  },
  {
    id: "workspace-knowledge",
    name: "Workspace Knowledge",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "📚",
    tagline: "Define shared coding standards once for every project.",
    description:
      "Set workspace-level rules, standards, and reference docs the agent applies across every project in the workspace.",
    capabilities: [
      "Workspace-scoped rule book",
      "Applies to every project automatically",
      "Pair with Knowledge Connectors (Fireflies, Gemini Enterprise, WordPress)",
    ],
    useCases: [
      "Enforce a house design system across an agency",
      "Codify accessibility standards once",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "groups-scim-sso",
    name: "Groups, SCIM & SAML SSO",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business / Enterprise",
    icon: "🔐",
    tagline: "Enterprise identity: groups, auto-provisioning, SAML.",
    description:
      "Group-based Access (with SCIM sync), SCIM provisioning, and SAML 2.0 SSO (Okta, Azure AD, OneLogin). Workspace provisioning bulk-adds users from a verified email domain.",
    capabilities: [
      "Group-based access control",
      "SCIM auto-provisioning / deprovisioning",
      "SAML 2.0 SSO (Okta, Azure AD, OneLogin)",
      "Workspace bulk provisioning from a domain",
    ],
    useCases: [
      "Roll Lovable out to a 500-person org",
      "Tie access to corporate IdP groups",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "custom-domain",
    name: "Custom Domains + In-App Purchase",
    category: "Deploy",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Paid plans",
    icon: "🌐",
    tagline: "Search, buy, and connect domains entirely in-app.",
    description:
      "Buy or connect custom domains directly inside Lovable. Stripe-powered checkout, automatic DNS configuration, automatic SSL via Let's Encrypt, optional www. subdomain setup, and workspace-level domain management. Updated May 6, 2026 with multi-domain primary controls.",
    capabilities: [
      "In-app domain search + Stripe checkout",
      "Auto DNS + SSL provisioning",
      "Workspace-level domain pool",
      "Set primary / connect multiple domains per project",
      "Auto-renew toggle, transfer between projects",
    ],
    useCases: [
      "Launch a marketing site on a fresh domain in 10 minutes",
      "Manage a portfolio of brand domains in one place",
    ],
    source: "https://docs.lovable.dev/features/custom-domain",
  },
  {
    id: "publishing-flow",
    name: "Guided Publishing Flow",
    category: "Deploy",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "🚀",
    tagline: "Multi-step publish with auto favicon and SEO.",
    description:
      "Guided multi-step publishing flow: automatic favicon cropping/conversion, reliable published-app links, and auto-generated SEO descriptions used for search results and link previews (Apr 2026).",
    capabilities: [
      "Auto favicon crop + format conversion",
      "Auto SEO description generation",
      "Reliable production URLs",
    ],
    useCases: [
      "Launch-ready SEO without manual config",
      "Polished social previews out of the box",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "lovable-cloud",
    name: "Lovable Cloud",
    category: "Deploy",
    status: "GA" as const,
    releaseDate: "2025-06-01",
    pricing: "All plans",
    icon: "☁️",
    tagline: "Managed backend with region choice and transactional email.",
    description:
      "Managed backend hosting (Supabase-compatible) with region selection (Americas, Europe, Asia Pacific), non-code deployments for secrets/storage, transactional app emails from your own domain, and CDN/proxy support (Cloudflare/CloudFront/Fastly).",
    capabilities: [
      "Region selection (US / EU / APAC)",
      "Secrets and storage management UI",
      "Transactional + auth emails from your domain",
      "CDN proxy support via CNAME",
      "Hourly resource alerts",
    ],
    useCases: [
      "EU data-residency for GDPR builds",
      "Production-ready hosting without DevOps",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "file-gen",
    name: "File Generation & Data Analysis",
    category: "Deploy",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Free, Pro, Business",
    icon: "📄",
    tagline: "Generate PDF, Excel, PPT files directly in chat.",
    description:
      "Run code and generate downloadable files (PDF, Excel, PPT) inside the chat. Available on Free, Pro, and Business plans.",
    capabilities: [
      "PDF, Excel, PPT generation",
      "Inline data analysis on attached datasets",
      "No project source changes",
    ],
    useCases: [
      "Hand a client a one-off PDF report",
      "Produce a quick Excel breakdown from a CSV",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-opus-47",
    name: "Claude Opus 4.7",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "🧠",
    tagline: "Default model for code quality and complex tasks.",
    description:
      "Lovable adopted Claude Opus 4.7 in April 2026 for improved code quality, instruction following, and performance on complex multi-file tasks. Replaces Opus 4.6 (Feb 2026).",
    capabilities: [
      "Higher code-quality ceiling",
      "Better long-context instruction following",
      "Stronger multi-file refactor reasoning",
    ],
    useCases: [
      "Long Agent Mode runs",
      "Plan Mode reasoning across a large codebase",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "gpt52",
    name: "GPT-5.2 Support",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "🟢",
    tagline: "GPT-5.2 selectable as agent backbone.",
    description: "Support for OpenAI's GPT-5.2 family as a model option for the Lovable agent.",
    capabilities: ["GPT-5.2 selectable in agent settings"],
    useCases: ["Compare outputs against Claude Opus on the same prompt"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "gemini3flash",
    name: "Gemini 3 Flash (default)",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "💎",
    tagline: "Fast default model for everyday builds.",
    description:
      "Google Gemini 3 Flash added and set as the default model. Pair with Nano Banana 2 (Gemini 3.1 Flash Image) for fast image generation (Mar 2026).",
    capabilities: [
      "Set as default agent model",
      "Pairs with Nano Banana 2 image model",
    ],
    useCases: [
      "Fast iteration loops on UI work",
      "Lower-cost agent runs",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "nano-banana-2",
    name: "Nano Banana 2 (Image Gen)",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "🍌",
    tagline: "Gemini 3.1 Flash Image — fast, transparent backgrounds, real in-image text.",
    description:
      "Google's Gemini 3.1 Flash Image model with transparent backgrounds and accurate in-image text rendering.",
    capabilities: [
      "Transparent background output",
      "Accurate text rendering inside images",
      "Fast turnaround",
    ],
    useCases: [
      "Logos and badges with real text",
      "Asset generation for landing pages",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "linkedin-vibe",
    name: "LinkedIn Vibe Coding Badge",
    category: "Community",
    status: "Beta",
    releaseDate: "2026-02-05",
    pricing: "All plans",
    icon: "💼",
    tagline: "L1–L5 vibe coding levels for your LinkedIn profile.",
    description:
      "Beta feature: vibe coding levels (L1 through L5) that can be added to your LinkedIn profile as a verified Lovable skill marker.",
    capabilities: [
      "L1–L5 progression",
      "LinkedIn profile badge",
      "Verified by Lovable activity",
    ],
    useCases: [
      "Signal builder credibility on LinkedIn",
      "Recruiter discovery of vibe coders",
    ],
    source: "https://docs.lovable.dev/tips-tricks/linkedin-certification",
  },
  {
    id: "lovable-mcp-server",
    name: "Lovable MCP Server",
    category: "Agent",
    status: "Beta",
    releaseDate: "2026-05-07",
    pricing: "Research Preview",
    icon: "🔌",
    tagline: "Connect external AI agents to Lovable via MCP.",
    description:
      "Research Preview: a remote MCP server that lets external AI agents and tools connect to Lovable using the Model Context Protocol — invoke project actions, read context, and orchestrate Lovable from outside the editor.",
    capabilities: [
      "Streamable HTTP MCP transport",
      "Drive Lovable from external agents",
      "Composable with other MCP clients",
    ],
    useCases: [
      "Trigger builds from a custom agent",
      "Wire Lovable into an internal AI workflow",
    ],
    source: "https://docs.lovable.dev/integrations/lovable-mcp-server",
  },
  {
    id: "build-with-url",
    name: "Lovable API: Build with URL",
    category: "Agent",
    status: "Beta",
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "🔗",
    tagline: "Kick off a Lovable build from a URL with prefilled context.",
    description:
      "A URL-based API that lets external apps and tools deep-link into Lovable with a prompt, repo, or attached context preloaded — useful for templates, partner integrations, and link-based onboarding.",
    capabilities: [
      "Prefilled prompt + parameters",
      "Lovable Link Generator helper",
      "Optional auth flow handoff",
    ],
    useCases: [
      "Branded 'Build in Lovable' buttons",
      "Curated template launchers",
    ],
    source: "https://docs.lovable.dev/integrations/build-with-url",
  },
  {
    id: "lovable-desktop-windows",
    name: "Lovable Desktop for Windows",
    category: "Workflow",
    status: "Beta",
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "🪟",
    tagline: "Native Windows build of Lovable Desktop — coming soon.",
    description:
      "macOS Lovable Desktop is GA; the Windows build is rolling out next, with the same multi-project tabs, keyboard shortcuts, and local MCP support (Figma Desktop, Paper).",
    capabilities: ["Multi-project tabs", "Local MCP servers", "Keyboard shortcuts"],
    useCases: ["Windows-based teams", "Local Figma/Paper workflows"],
    source: "https://docs.lovable.dev/integrations/desktop-app",
  },
  {
    id: "lovable-bonuses",
    name: "Lovable Bonuses (Credits)",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2026-01-15",
    pricing: "All plans",
    icon: "🎁",
    tagline: "Earn credits — daily messaging, custom domains, invites.",
    description:
      "Earn extra credits: send 25 daily messages for 5 credits, plus one-time 5-credit bonuses for adding a custom domain or inviting a collaborator (new users since Jan 15, 2026).",
    capabilities: [
      "Daily 5-credit bonus (25 messages)",
      "5-credit custom domain bonus",
      "5-credit collaborator invite bonus",
    ],
    useCases: [
      "Stretch credits while learning",
      "Onboarding incentive for new workspaces",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "build-credits-topup",
    name: "Build Credit Top-ups",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "Pro / Business",
    icon: "💰",
    tagline: "Buy 50-credit packs, valid 12 months.",
    description: "Top up Pro and Business plans in 50-credit increments. Credits valid for 12 months.",
    capabilities: ["50-credit packs", "12-month validity"],
    useCases: ["Burst capacity for a launch sprint"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "lovable-ai-gateway",
    name: "Lovable AI Gateway",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2025-09-01",
    pricing: "Usage-based, free tier",
    icon: "🧠",
    tagline: "Built-in access to frontier models — no API keys needed.",
    description:
      "Lovable AI lets your app call leading LLMs and image models without managing API keys or provider accounts. Powers chatbots, summaries, sentiment, document Q&A, multilingual translation, and image/document analysis. Requires Lovable Cloud.",
    capabilities: ["No API keys", "Frontier text + image models", "Free monthly usage", "Server-side from Cloud functions"],
    useCases: ["AI summaries", "Conversational chatbots", "Doc Q&A", "Image generation in-app"],
    source: "https://docs.lovable.dev/features/ai",
  },
  {
    id: "paddle-payments",
    name: "Paddle Payments",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Paid plans",
    icon: "💳",
    tagline: "One-click subscriptions and one-time checkout via Paddle.",
    description:
      "Lovable Payments adds Paddle alongside Stripe — handles account creation, webhooks, subscription data, and a working test checkout in minutes. Supports SaaS, subscriptions, memberships, and digital products.",
    capabilities: ["Hosted checkout", "Subscriptions + one-time", "Auto webhook wiring", "Same-day go-live"],
    useCases: ["SaaS billing", "Memberships", "Digital product sales"],
    source: "https://docs.lovable.dev/features/payments",
  },
  {
    id: "app-emails",
    name: "App Emails (Transactional)",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Paid plans (Cloud)",
    icon: "✉️",
    tagline: "Branded transactional email from your own domain.",
    description:
      "Send order confirmations, receipts, shipping updates, and security notices from your verified domain. Branded templates, automatic unsubscribe handling, suppression lists, and delivery infra all built-in — no external email provider required.",
    capabilities: ["Custom domain sender", "Branded templates", "Auto unsubscribe", "Suppression lists"],
    useCases: ["Order confirmations", "User notifications", "Receipts"],
    source: "https://docs.lovable.dev/features/custom-emails",
  },
  {
    id: "auth-emails",
    name: "Authentication Emails",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Paid plans (Cloud)",
    icon: "🔐",
    tagline: "Send signup, magic link, and reset emails from your domain.",
    description:
      "Send authentication emails from your own domain instead of the default Cloud sender. Lovable manages DNS, SPF, DKIM, and DMARC automatically. Covers signup confirmations, password resets, magic links, invitations, email-change confirmations, and reauthentication.",
    capabilities: ["Auto DNS/SPF/DKIM/DMARC", "Custom branding", "Magic links + OTP", "All auth flows"],
    useCases: ["Brand-consistent login", "Improved deliverability"],
    source: "https://docs.lovable.dev/features/custom-emails",
  },
  {
    id: "saml-sso-apps",
    name: "SAML 2.0 SSO for Your Apps",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Cloud apps",
    icon: "🪪",
    tagline: "Add enterprise SSO to apps you build.",
    description:
      "Add SAML 2.0 SSO to Cloud apps from Lovable Cloud → Users → Auth, or just ask the agent. Works with Okta, Azure AD / Entra ID, OneLogin, and other SAML 2.0 providers — Lovable collects metadata and configures auth automatically.",
    capabilities: ["Okta, Entra, OneLogin", "Auto IdP metadata setup", "Domain-bound auth"],
    useCases: ["Sell to enterprise", "Internal tools with corp SSO"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "aikido-connector",
    name: "Aikido Pentesting",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Connector",
    icon: "🛡️",
    tagline: "AI-powered penetration testing wired into Security view.",
    description:
      "Aikido brings dynamic AI pentesting to Lovable. Findings sync into the project's Security view, and Aikido produces shareable reports for security reviews and compliance.",
    capabilities: ["Dynamic exploit testing", "Security view sync", "Shareable reports"],
    useCases: ["Pre-launch security audit", "Compliance evidence"],
    source: "https://docs.lovable.dev/integrations/aikido",
  },
  {
    id: "storyblok-connector",
    name: "Storyblok CMS",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Connector",
    icon: "📚",
    tagline: "Headless CMS via Storyblok Content Delivery API.",
    description:
      "Connect a Storyblok space to fetch published stories, drafts, and assets through the gateway. Great for landing pages, blogs, product catalogs, and docs powered by your CMS.",
    capabilities: ["Stories + drafts", "Pagination + filters", "Auto-handled tokens via gateway"],
    useCases: ["Marketing site CMS", "Catalog content", "Docs"],
    source: "https://docs.lovable.dev/integrations/storyblok",
  },
  {
    id: "versioning-v2",
    name: "Versioning 2.0",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-03-03",
    pricing: "All plans",
    icon: "🕓",
    tagline: "Improved history, restore points, and version tracking.",
    description:
      "Versioning 2.0 brings better history, named restore points, and clearer tracking across iterations so you can move fast without losing work.",
    capabilities: ["Restore points", "Inline diff history", "Stable rollback"],
    useCases: ["Recover from bad edits", "Compare iterations"],
    source: "https://lovable.dev/blog/product-updates/versioning-with-lovable-two-point-zero",
  },
  {
    id: "workspace-invite-links",
    name: "Workspace Invite Links",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Free / Pro / Business",
    icon: "🔗",
    tagline: "Share role-based, time-limited invite links.",
    description:
      "Editors and above can invite people via shareable role-based links instead of typing emails. Links expire after 5 days; one active link per role at a time.",
    capabilities: ["Role-based links", "5-day expiry", "Regenerate / disable"],
    useCases: ["Onboard a class or cohort", "Bulk team invites"],
    source: "https://docs.lovable.dev/features/workspace",
  },
  {
    id: "bulk-member-mgmt",
    name: "Bulk Member Management",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "👥",
    tagline: "Select-mode for editing many members at once.",
    description:
      "Workspace admins and owners can change roles, set credit limits, remove users, or revoke invitations for many members in a single action.",
    capabilities: ["Select mode", "Bulk role change", "Bulk credit limits"],
    useCases: ["Quarterly access reviews", "Large workspace cleanups"],
    source: "https://docs.lovable.dev/features/workspace",
  },
  {
    id: "domain-jit",
    name: "Domain JIT Provisioning",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business+",
    icon: "🏢",
    tagline: "Auto-add verified-email users into the right workspace.",
    description:
      "New users signing up with a verified company email are auto-placed into the correct workspaces with the configured default role. Workspace discovery lets users request access during onboarding.",
    capabilities: ["JIT placement", "Default role on join", "Workspace discovery"],
    useCases: ["Frictionless org onboarding", "Self-serve access"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "image-tools",
    name: "Image Tools (Zoom & Crop)",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "🔍",
    tagline: "Zoom and crop uploaded images directly in chat.",
    description:
      "The agent can zoom into and crop specific regions of uploaded images so you can reference UI details and guide it with precise visual context.",
    capabilities: ["In-chat zoom", "Crop to region", "Pass cropped context to agent"],
    useCases: ["Pinpoint a UI bug", "Reference an icon detail"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "command-palette",
    name: "Command Palette (⌘K)",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "⌘",
    tagline: "Search projects, settings, and Cloud from the keyboard.",
    description:
      "Redesigned ⌘K palette: search projects, tools, settings, switch workspaces, browse folders, and access Lovable Cloud features without leaving the keyboard.",
    capabilities: ["Project + folder search", "Workspace switcher", "Cloud actions", "Connector access"],
    useCases: ["Power-user navigation", "Fast project switching"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "cdn-reverse-proxy",
    name: "CDN / Reverse Proxy Domains",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Paid plans",
    icon: "🌐",
    tagline: "Connect domains routed through Cloudflare, CloudFront, or Fastly.",
    description:
      "Reintroduced support for custom domains routed through your own CDN or reverse proxy, with a CNAME-based config and Lovable-handled SSL once the record is live.",
    capabilities: ["CNAME-based setup", "Auto SSL", "Cloudflare/CloudFront/Fastly compatible"],
    useCases: ["Apps behind Cloudflare", "Multi-region edge"],
    source: "https://docs.lovable.dev/features/custom-domain",
  },
  {
    id: "project-visibility",
    name: "Project Visibility & Public Remixing",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "👁️",
    tagline: "Workspace-default visibility with opt-in public remixing.",
    description:
      "Public project visibility was removed; all projects default to workspace visibility with access controlled via project access settings. Enable Public Remixing in Project Settings to let others copy and remix your project.",
    capabilities: ["Workspace-only by default", "Per-project access controls", "Opt-in remix"],
    useCases: ["Safer defaults", "Templates and showcases"],
    source: "https://docs.lovable.dev/features/project-visibility",
  },
  {
    id: "autonomous-complex-builds",
    name: "Autonomous Complex Builds",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2026-02-15",
    pricing: "All plans",
    icon: "🛠️",
    tagline: "Multi-step builds handled end-to-end without hand-holding.",
    description:
      "The agent now plans, executes, and verifies multi-step builds autonomously — chaining edits, migrations, and integrations across the codebase before reporting back.",
    capabilities: [
      "Long-horizon task planning",
      "Cross-file edits and migrations",
      "Self-verification before completion",
      "Mid-build error recovery",
    ],
    useCases: [
      "Ship a full feature from a single prompt",
      "Run multi-table schema changes safely",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "chat-mode-clarifying-questions",
    name: "Plan Before You Build — Chat Mode Questions",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2025-12-15",
    pricing: "1 credit per message",
    icon: "❓",
    tagline: "Chat Mode now asks clarifying questions to reduce guessing.",
    description:
      "Chat Mode interrogates ambiguous prompts before the agent commits to an approach — surfacing missing context, conflicting requirements, and edge cases up front.",
    capabilities: [
      "Targeted clarifying questions",
      "Context gap detection",
      "Lower wasted credits on rework",
    ],
    useCases: [
      "Scope features before kicking off Agent Mode",
      "Avoid rebuilds caused by misread intent",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-opus-45-chat",
    name: "Claude Opus 4.5 Early Access in Chat Mode",
    category: "AI Models",
    status: "Beta",
    releaseDate: "2025-11-12",
    pricing: "Pro and Business plans",
    icon: "🧠",
    tagline: "Test Anthropic's newest reasoning model in Chat Mode.",
    description:
      "Opt-in early access to Claude Opus 4.5 inside Chat Mode for deeper reasoning, longer-horizon planning, and stronger code understanding on complex projects.",
    capabilities: [
      "Frontier reasoning quality",
      "Available in Chat Mode only",
      "Switchable per conversation",
    ],
    useCases: [
      "Architect complex features",
      "Diagnose subtle bugs across many files",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "themes-design-view",
    name: "Themes and Design View",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-11-15",
    pricing: "All plans",
    icon: "🎨",
    tagline: "Themes and Visual Edits unified in the new Design view.",
    description:
      "A consolidated Design surface where theme tokens, typography, and visual edits live in one place — switch palettes, tweak components, and preview live.",
    capabilities: [
      "Unified theme + visual edits",
      "Live preview across the app",
      "Token-driven design system",
    ],
    useCases: [
      "Restyle an entire app in seconds",
      "Iterate brand systems without code",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "revert-edit-messages",
    name: "Revert and Edit Messages",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-11-22",
    pricing: "All plans",
    icon: "↩️",
    tagline: "Roll back to any prior message or edit and re-run.",
    description:
      "Restore is now Revert — jump back to any earlier point in a conversation, edit the prompt, and re-run from that state without losing later context.",
    capabilities: [
      "Revert to any message",
      "Edit prompts in place",
      "Branch from prior states",
    ],
    useCases: [
      "Recover from a wrong agent decision",
      "Try alternate prompts from the same baseline",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "workspace-publishing",
    name: "Workspace Publishing",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-11-25",
    pricing: "Business and Enterprise plans",
    icon: "🏢",
    tagline: "Authenticated org-scoped publishing without extra setup.",
    description:
      "Publish projects behind workspace authentication — only members of the org can access the published app, with no custom auth wiring required.",
    capabilities: [
      "Org-only access by default",
      "Zero auth configuration",
      "Member-managed visibility",
    ],
    useCases: [
      "Ship internal tools safely",
      "Share work-in-progress with the team only",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "new-project-layout",
    name: "New Project Layout",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-10-05",
    pricing: "All plans",
    icon: "🧭",
    tagline: "Analytics, Speed, and Security live in the project view switcher.",
    description:
      "The project view reorganized so Analytics, Speed, and Security each get a dedicated surface — switch contexts without leaving the project.",
    capabilities: [
      "Dedicated Analytics view",
      "Dedicated Speed view",
      "Dedicated Security view",
    ],
    useCases: [
      "Monitor app health in one place",
      "Triage performance and security findings",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "multi-format-file-uploads",
    name: "Multi-Format File Uploads",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-09-12",
    pricing: "All plans",
    icon: "📎",
    tagline: "CSV, PDF, TTF, MP4, and image uploads as build context.",
    description:
      "Drop files of any common format into a prompt — the agent reads them as first-class context for builds, designs, and data work.",
    capabilities: [
      "CSV, PDF, TTF, MP4, image support",
      "Files become agent context",
      "Drag-and-drop into chat",
    ],
    useCases: [
      "Seed a build from a spec PDF",
      "Generate UI from a font and brand assets",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "voice-mode",
    name: "Voice Mode",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-09-20",
    pricing: "All plans",
    icon: "🎙️",
    tagline: "Describe what you want to build in speech instead of typing.",
    description:
      "Press to talk — Voice Mode transcribes and routes your spoken intent directly into the agent, ideal for long descriptions and on-the-go iteration.",
    capabilities: [
      "Push-to-talk capture",
      "Real-time transcription",
      "Routes into Chat or Agent mode",
    ],
    useCases: [
      "Brainstorm a build hands-free",
      "Dictate long product specs",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "automatic-in-app-seo",
    name: "Automatic In-App SEO",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2025-08-12",
    pricing: "All plans",
    icon: "🔍",
    tagline: "Auto-optimized meta, structured data, alt text, URLs, and sitemap.",
    description:
      "Published apps ship with sane SEO defaults out of the box — meta tags, JSON-LD, image alt text, clean URLs, and a generated sitemap update as the app changes.",
    capabilities: [
      "Auto meta and Open Graph",
      "Structured data (JSON-LD)",
      "Image alt text generation",
      "Sitemap and clean URLs",
    ],
    useCases: [
      "Rank a marketing site without manual SEO work",
      "Ship indexable content from day one",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "realtime-security-protection",
    name: "Real-Time Security Protection",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2025-08-15",
    pricing: "All plans",
    icon: "🛡️",
    tagline: "Real-time risk detection plus SOC 2 Type II and ISO 27001 certified.",
    description:
      "Continuous risk detection across builds and runtime, backed by SOC 2 Type II and ISO 27001 certifications for the platform.",
    capabilities: [
      "Real-time threat detection",
      "SOC 2 Type II certified",
      "ISO 27001 certified",
    ],
    useCases: [
      "Meet enterprise security requirements",
      "Catch risky patterns before publish",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "student-discount-50",
    name: "Student Discount 50%",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2025-08-20",
    pricing: "All plans",
    icon: "🎓",
    tagline: "50% off paid plans with student verification.",
    description:
      "Verified students get 50% off any paid Lovable plan — verification handled inline at checkout.",
    capabilities: [
      "Inline student verification",
      "Applies to all paid plans",
      "Renewable annually",
    ],
    useCases: [
      "Build production projects on a student budget",
      "Class and club projects",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "credit-rollovers",
    name: "Credit Rollovers",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-08-25",
    pricing: "All plans",
    icon: "🔄",
    tagline: "Monthly rollover up to your limit; annual rollover up to 12x.",
    description:
      "Unused credits now roll forward — monthly up to your plan's credit limit and annually up to 12x your monthly allotment.",
    capabilities: [
      "Monthly rollover up to plan limit",
      "Annual rollover up to 12x monthly",
      "Automatic, no action needed",
    ],
    useCases: [
      "Save credits for big build pushes",
      "Smooth out uneven usage months",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "series-a-200m",
    name: "Lovable $200M Series A",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2025-07-15",
    pricing: "All plans",
    icon: "🚀",
    tagline: "$200M Series A led by Accel.",
    description:
      "Lovable raised a $200M Series A led by Accel, with Creandum, 20VC, Hummingbird, byFounders, and Visionaries participating — fueling the next phase of the platform.",
    capabilities: [
      "Accelerated platform investment",
      "Expanded team and infra",
      "Long-term roadmap funded",
    ],
    useCases: [
      "Build with confidence on a well-capitalized platform",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "lovable-2-agent-default",
    name: "Agent Mode Default in Lovable 2.0",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2025-07-20",
    pricing: "All plans",
    icon: "🌟",
    tagline: "Lovable 2.0 makes Agent Mode the default agent-first interface.",
    description:
      "Lovable 2.0 ships Agent Mode as the default experience — an agent-first interface for deploying, managing, and evolving projects end-to-end.",
    capabilities: [
      "Agent Mode default for new projects",
      "Unified deploy and manage flows",
      "Foundation for autonomous workflows",
    ],
    useCases: [
      "Run an entire project from a single agent thread",
      "Hand off ongoing maintenance to the agent",
    ],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-code-mcp",
    name: "Lovable MCP for Claude Code",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-01-15",
    pricing: "All plans",
    icon: "✦",
    tagline:
      "Use Lovable as an MCP server inside Claude Code so the agent can build, edit, and deploy your projects directly.",
    description:
      "The Lovable MCP server lets external AI agents — including Claude Code, Cursor, and any MCP-compatible client — call Lovable as a tool. Configure it once with your Lovable API key, then have the agent create or modify Lovable projects, run tasks, and ship deploys from the same conversation where it edits your local code.",
    capabilities: [
      "Run Lovable as an MCP server inside Claude Code",
      "Bidirectional context between Claude Code and Lovable",
      "Authenticated with a per-user Lovable API key",
      "Compatible with any MCP-spec client (Cursor, Windsurf, OpenAI Agents)",
    ],
    useCases: [
      "Have Claude Code orchestrate a Lovable build from your local repo",
      "Wire Lovable into a multi-agent IDE workflow",
      "Trigger Lovable deploys from a CLI agent",
    ],
    source: "https://docs.lovable.dev/integrations/lovable-mcp-server#claude-code",
  },
  {
    id: "lovable-payments",
    name: "Lovable Payments",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Pro and Business plans",
    icon: "",
    tagline: "Built-in subscriptions and one-time payments via Paddle or Stripe.",
    description: "Lovable Payments handles account creation, webhooks, and subscription management for monetized apps. Developers choose Paddle (Merchant of Record handling tax and invoicing) or Stripe (standard PSP pricing with lower per-transaction fees). Requires Lovable Cloud for webhook data.",
    capabilities: ["Subscription tiers and one-time purchases", "Free trials and discount codes", "Customer portal and revenue analytics", "Test and live environment isolation"],
    useCases: ["SaaS subscription billing", "One-time digital purchases"],
    source: "https://docs.lovable.dev/features/payments",
  },
  {
    id: "lovable-desktop-app",
    name: "Lovable Desktop App",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Native macOS app with local MCP support and multi-project tabs.",
    description: "The Lovable Desktop App runs natively on macOS (Apple Silicon and Intel) with Windows support announced as coming soon. It is the only surface that supports local MCP servers — connecting to tools running directly on the machine like Figma Desktop and Paper — and adds tab-based multi-project management with keyboard shortcuts.",
    capabilities: ["Local MCP server connections (Figma Desktop, Paper)", "Multi-project tabs with Cmd+T and Cmd+1-9", "Command palette and keyboard shortcuts"],
    useCases: ["Designers connecting Figma Desktop while building", "Power users managing several projects at once"],
    source: "https://docs.lovable.dev/integrations/desktop-app",
  },
  {
    id: "tasks-agent-transparency",
    name: "Tasks (Agent Transparency)",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Live task list showing what the agent is doing during a run.",
    description: "Tasks surfaces the agent's working plan as a live, structured list so users can watch which step the agent is on and what is queued. Improves transparency during multi-step generations.",
    capabilities: ["Live task list rendered during agent runs", "Visibility into multi-step plans"],
    useCases: ["Following along during long autonomous runs", "Debugging where an agent got stuck"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "file-references-at-mentions",
    name: "File References (@ Mentions)",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Mention code files directly in chat with @ to scope agent context.",
    description: "Typing @ in chat lets users pick specific code files to reference, scoping the agent's reads and edits to the chosen files rather than the whole project.",
    capabilities: ["@-mention any file in the project", "Scope agent reads to selected files"],
    useCases: ["Refactoring a single component", "Asking targeted questions about one file"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "test-and-live-environments",
    name: "Test and Live Environments",
    category: "Cloud",
    status: "Removed" as const,
    releaseDate: "2026-02-05",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Separate dev and production Cloud environments — removed 2026-03-24.",
    description: "Test and Live Environments provided isolated dev and prod databases inside Lovable Cloud with one-way data copies and automatic backups before each publish. Discontinued for new projects as of March 24, 2026; existing Cloud projects retained access but cannot re-enable once removed.",
    capabilities: ["Isolated test and live databases", "Automatic Live backups before publish", "Safe schema migrations"],
    useCases: ["Staging schema changes before production", "Testing without touching live data"],
    source: "https://docs.lovable.dev/features/environments",
  },
  {
    id: "code-execution-generate-files",
    name: "Generate Files and Data Analysis",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Run Python/Node in chat to analyze data and produce downloadable files.",
    description: "Generate Files runs scripts in an isolated Linux environment with pandas, matplotlib, and psql preinstalled, returning downloadable outputs (PDF, DOCX, PPTX, XLSX, CSV, MP4, MP3, etc.) without modifying the project's source code. Max 20 MB per file. Cloud database access available when permissions are set to Always allow.",
    capabilities: ["Python and Node code execution", "Output PDF, DOCX, PPTX, XLSX, images, MP4, MP3", "Query Cloud database from chat"],
    useCases: ["Ad-hoc data analysis on uploaded CSVs", "Generating one-off reports as PDF"],
    source: "https://docs.lovable.dev/features/generate-files",
  },
  {
    id: "claude-opus-4-7",
    name: "Claude Opus 4.7",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Paid plans",
    icon: "",
    tagline: "Anthropic Opus 4.7 available in Lovable chat.",
    description: "Opus 4.7 became selectable in Lovable's agent and chat alongside other premium models as part of the April 2026 release.",
    capabilities: ["Premium reasoning for complex builds"],
    useCases: ["Architecturally heavy code generation"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-opus-4-6",
    name: "Claude Opus 4.6",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "Paid plans",
    icon: "",
    tagline: "Anthropic Opus 4.6 model option in Lovable.",
    description: "Opus 4.6 was added as a model option in the Feb 23 2026 release. Routed through Lovable's agent for complex code generation.",
    capabilities: ["Selectable in agent and chat"],
    useCases: ["High-quality code edits"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-sonnet-4-5",
    name: "Claude Sonnet 4.5",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "Paid plans",
    icon: "",
    tagline: "Sonnet 4.5 model support for faster, cheaper builds.",
    description: "Sonnet 4.5 support was added alongside the Lovable Cloud launch on Oct 6, 2025 as a balanced cost/quality option.",
    capabilities: ["Balanced cost and quality"],
    useCases: ["Everyday agent prompts"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "claude-4",
    name: "Claude 4",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2025-05-24",
    pricing: "Paid plans",
    icon: "",
    tagline: "Initial Claude 4 integration — 91% error reduction and 40% faster.",
    description: "Claude 4 landed on May 24, 2025 as the new default coding model and was credited with a 91% error reduction and 40% speed improvement over the prior model.",
    capabilities: ["Default for full-stack generation"],
    useCases: ["General app builds"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "gpt-5-2",
    name: "GPT-5.2",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "Paid plans",
    icon: "",
    tagline: "OpenAI GPT-5.2 support in Lovable.",
    description: "GPT-5.2 became available as a selectable model on Jan 16, 2026 alongside Gemini 3 Flash. Accessible via Lovable AI usage-based pricing.",
    capabilities: ["Selectable in chat and agent"],
    useCases: ["OpenAI-optimized prompts"],
    source: "https://docs.lovable.dev/integrations/ai",
  },
  {
    id: "gpt-5-4",
    name: "GPT-5.4",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "Paid plans",
    icon: "",
    tagline: "GPT-5.4 standard, Pro, Mini, and Nano variants.",
    description: "GPT-5.4 family — including Pro, standard, Mini, and Nano variants — is selectable in Lovable AI for builds requiring OpenAI-class reasoning.",
    capabilities: ["Multiple cost/quality tiers (Pro, standard, Mini, Nano)"],
    useCases: ["Tuning cost-per-build across teams"],
    source: "https://docs.lovable.dev/integrations/ai",
  },
  {
    id: "gpt-5-5",
    name: "GPT-5.5 (Pro and Standard)",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "Paid plans",
    icon: "",
    tagline: "OpenAI GPT-5.5 Pro and standard tiers in Lovable AI.",
    description: "GPT-5.5 Pro and standard tiers are documented as supported in Lovable AI alongside the rest of the OpenAI lineup.",
    capabilities: ["Pro and standard tiers selectable"],
    useCases: ["Top-tier OpenAI generation"],
    source: "https://docs.lovable.dev/integrations/ai",
  },
  {
    id: "gemini-3-flash",
    name: "Gemini 3 Flash",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "",
    tagline: "Default Lovable AI model — Gemini 3 Flash.",
    description: "Gemini 3 Flash is the default model for Lovable AI workloads. Free tier $1 monthly AI budget allows usage without configuration.",
    capabilities: ["Default Lovable AI model", "Free tier compatible"],
    useCases: ["AI features inside published apps"],
    source: "https://docs.lovable.dev/integrations/ai",
  },
  {
    id: "gemini-3-pro",
    name: "Gemini 3 Pro",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "Paid plans",
    icon: "",
    tagline: "Gemini 3 Pro model support.",
    description: "Gemini 3 Pro support was added in the Nov 26, 2025 release for higher-quality Google-model builds.",
    capabilities: ["Google-class reasoning"],
    useCases: ["Long-context tasks"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "gemini-3-1-pro",
    name: "Gemini 3.1 Pro",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Paid plans",
    icon: "",
    tagline: "Latest Google Gemini 3.1 Pro tier in Lovable AI.",
    description: "Gemini 3.1 Pro is listed as a selectable model in the Lovable AI integration.",
    capabilities: ["Selectable in Lovable AI"],
    useCases: ["High-quality Google-model generations"],
    source: "https://docs.lovable.dev/integrations/ai",
  },
  {
    id: "nano-banana-2",
    name: "Nano Banana 2 (Gemini 3.1 Flash Image)",
    category: "AI Models",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Paid plans",
    icon: "",
    tagline: "Gemini 3.1 Flash Image — Nano Banana 2 — for in-app image generation.",
    description: "Nano Banana 2 (Google's Gemini 3.1 Flash Image) is supported for image generation in builds and design tools.",
    capabilities: ["In-app image generation"],
    useCases: ["Generating product imagery in apps"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "cloud-database",
    name: "Cloud Database",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "Usage-based (paid plans)",
    icon: "",
    tagline: "Managed Postgres with natural-language schema generation.",
    description: "Lovable Cloud's database service provides managed Postgres with auto-generated schemas from plain-language descriptions and a UI for managing data without SQL. Available in three hosting regions with instance scaling.",
    capabilities: ["Natural-language schema generation", "UI-based data management", "Three regions (Americas, Europe, Asia Pacific)"],
    useCases: ["Persisting user data without SQL", "Multi-region deployment"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-auth-email-phone",
    name: "Cloud Authentication (Email and Phone)",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "Usage-based (paid plans)",
    icon: "",
    tagline: "Built-in email and phone sign-in for Cloud apps.",
    description: "Lovable Cloud provides built-in signup and login flows supporting email and phone authentication out of the box, with managed user accounts and encryption.",
    capabilities: ["Email-based auth", "Phone-based auth", "Managed user accounts"],
    useCases: ["Standing up a user system without writing auth code"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-auth-google",
    name: "Cloud Google Authentication",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "OAuth 2.0 Google sign-in — managed by Lovable or BYO credentials.",
    description: "Google OAuth sign-in for Cloud apps. Default 'Managed by Lovable' mode requires no Google Cloud setup; an advanced mode lets developers bring their own OAuth credentials with custom consent branding and scopes.",
    capabilities: ["Lovable-managed OAuth (zero config)", "Self-managed credentials option", "Custom scopes in self-managed mode"],
    useCases: ["Frictionless social login", "Enterprise consent-screen branding"],
    source: "https://docs.lovable.dev/features/google-auth",
  },
  {
    id: "cloud-auth-apple",
    name: "Cloud Apple Authentication",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Apple Sign-In for Cloud apps.",
    description: "Apple Sign-In was added alongside Google authentication in the Feb 5, 2026 release as a seamless social login option for Cloud apps.",
    capabilities: ["Apple OAuth sign-in"],
    useCases: ["iOS-facing app login"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "cloud-saml-sso-for-apps",
    name: "Cloud SAML SSO (Per-App)",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Let end users of your Cloud app sign in through their IdP.",
    description: "Per-project SAML 2.0 SSO for end users of Cloud apps. Supports Okta, Microsoft Entra ID, Google Workspace, OneLogin, JumpCloud, and Auth0 via metadata URL. Includes JIT provisioning and domain-based routing. One SAML provider per project; SP-initiated only; no SCIM or attribute mapping UI.",
    capabilities: ["Any SAML 2.0 IdP via metadata URL", "JIT user provisioning on first login", "Domain-based routing", "Certificate auto-rotation"],
    useCases: ["B2B apps requiring customer SSO", "Enterprise procurement requirements"],
    source: "https://docs.lovable.dev/features/cloud-saml-sso",
  },
  {
    id: "cloud-storage",
    name: "Cloud Storage",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "Usage-based (paid plans)",
    icon: "",
    tagline: "Secure bucket storage up to 2 GB per file.",
    description: "Lovable Cloud Storage provides secure file storage with public or private bucket controls and a 2 GB per-file limit. Bucket and folder deletion shipped April 2026.",
    capabilities: ["Public or private buckets", "2 GB per-file limit", "Bucket and folder deletion"],
    useCases: ["User-uploaded images and documents", "Media-heavy apps"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-edge-functions",
    name: "Cloud Edge Functions",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "Usage-based (paid plans)",
    icon: "",
    tagline: "Serverless compute for APIs, webhooks, and scheduled tasks.",
    description: "Edge Functions provide serverless compute for custom logic — APIs, webhooks, email notifications, payment processing, and scheduled tasks — that scales automatically.",
    capabilities: ["API and webhook endpoints", "Scheduled tasks", "Auto-scaling"],
    useCases: ["Stripe webhooks", "Cron jobs in apps"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-region-selection",
    name: "Cloud Region Selection",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Pick Americas, Europe, or Asia Pacific hosting region for Cloud projects.",
    description: "Cloud Region Selection lets workspace admins pick the hosting region (Americas, Europe, or Asia Pacific) for a Cloud project. Region defaults to user location.",
    capabilities: ["Three regions: Americas, Europe, Asia Pacific", "Automatic default by user location"],
    useCases: ["EU data residency requirements", "Latency optimization"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-auto-top-up",
    name: "Cloud Auto Top-Up",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-11-05",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Automatic Cloud and AI top-ups with monthly caps.",
    description: "Auto Top-Up keeps Cloud and AI usage running without manual intervention. Workspace admins set monthly caps to bound spend; top-ups can be one-time ($10–$1,000) or automatic.",
    capabilities: ["Automatic balance top-ups", "Monthly spend caps", "One-time top-ups $10–$1,000"],
    useCases: ["Production apps that can't run out of credits", "Bounded auto-replenishment"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "cloud-instance-scaling",
    name: "Cloud Instance Scaling",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "Lovable Cloud only",
    icon: "",
    tagline: "Scale Cloud database/compute from Tiny to Large.",
    description: "Cloud instances scale across size tiers (Tiny to Large) for performance needs. Real-time monitoring, debugging logs, and alerts when resource limits approach are included.",
    capabilities: ["Tiny to Large instance tiers", "Real-time monitoring and logs", "Resource-limit alerts"],
    useCases: ["Scaling production apps under load", "Cost-tuning small projects"],
    source: "https://docs.lovable.dev/integrations/cloud",
  },
  {
    id: "stripe-connector",
    name: "Stripe Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-10-06",
    pricing: "All plans",
    icon: "",
    tagline: "Handle payments and subscriptions through Stripe.",
    description: "Stripe app connector enables payment processing, subscription billing, and webhook handling. Updated integration shipped with the Lovable Cloud release.",
    capabilities: ["Payment processing", "Subscription billing", "Webhook handling"],
    useCases: ["E-commerce checkout", "SaaS subscriptions"],
    source: "https://docs.lovable.dev/integrations/stripe",
  },
  {
    id: "supabase-connector",
    name: "Supabase Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans",
    icon: "",
    tagline: "Managed Postgres, auth, storage, realtime, and Edge Functions via Supabase.",
    description: "Supabase integration provides a fully managed backend — PostgreSQL with SQL Assistant, OAuth and email auth, 50MB file uploads on free tier, real-time subscriptions, and Edge Functions for custom server logic.",
    capabilities: ["Postgres with natural-language schema", "Auth (email + OAuth)", "Storage and real-time subscriptions", "Edge Functions"],
    useCases: ["Apps needing more than Lovable Cloud offers", "Real-time chat and dashboards"],
    source: "https://docs.lovable.dev/integrations/supabase",
  },
  {
    id: "github-connector",
    name: "GitHub Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans (Enterprise Cloud/Server: Enterprise plan)",
    icon: "",
    tagline: "Two-way sync between Lovable and GitHub repositories.",
    description: "Bidirectional sync with github.com (all plans), GitHub Enterprise Cloud, and GitHub Enterprise Server (both Enterprise plan). Supports branching, PRs, code reviews, repository transfer, and co-author tracking.",
    capabilities: ["Bidirectional sync", "Pull requests and branches", "GitHub Enterprise Cloud/Server (Enterprise plan)", "Co-author tracking"],
    useCases: ["Local IDE development", "Code review workflows"],
    source: "https://docs.lovable.dev/integrations/github",
  },
  {
    id: "gitlab-connector",
    name: "GitLab Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Two-way sync with GitLab.com and self-managed GitLab.",
    description: "GitLab integration supports both GitLab.com and self-managed instances with bidirectional sync on the default branch only. Export-only — existing GitLab repos cannot be imported into Lovable.",
    capabilities: ["GitLab.com and self-managed support", "Default-branch bidirectional sync", "Merge requests and reviews"],
    useCases: ["Teams on GitLab self-managed", "Hybrid Lovable + IDE workflows"],
    source: "https://docs.lovable.dev/integrations/gitlab",
  },
  {
    id: "shopify-connector",
    name: "Shopify Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-11-05",
    pricing: "All plans",
    icon: "",
    tagline: "Build Shopify stores conversationally with per-user connections.",
    description: "Shopify app connector lets users build online stores through conversational AI with integrated Shopify setup, per-user connections with auto-refresh, and a disconnect option.",
    capabilities: ["Conversational store building", "Per-user connections with auto-refresh", "Store disconnect"],
    useCases: ["Spinning up branded storefronts", "Custom Shopify frontends"],
    source: "https://docs.lovable.dev/integrations/shopify",
  },
  {
    id: "resend-connector",
    name: "Resend Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Send transactional and marketing email via Resend.",
    description: "Resend connector sends transactional and marketing emails from app workflows. Promoted to a first-class connector in the April 2026 connector wave.",
    capabilities: ["Transactional email", "Marketing email"],
    useCases: ["Order confirmations", "Newsletter delivery"],
    source: "https://docs.lovable.dev/integrations/resend",
  },
  {
    id: "elevenlabs-connector",
    name: "ElevenLabs Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Audio generation and text-to-speech via ElevenLabs.",
    description: "ElevenLabs app connector adds audio generation and TTS to Lovable apps. ElevenLabs Scribe V2 transcription added Jan 2026.",
    capabilities: ["Text-to-speech generation", "Audio generation", "Scribe V2 transcription"],
    useCases: ["Voice features in apps", "Audio narration"],
    source: "https://docs.lovable.dev/integrations/eleven-labs",
  },
  {
    id: "perplexity-connector",
    name: "Perplexity Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Web-backed AI research and Q&A via Perplexity.",
    description: "Perplexity connector answers questions and runs web-backed research from inside Lovable apps.",
    capabilities: ["Web-backed Q&A", "Research with citations"],
    useCases: ["Research assistants in apps", "Up-to-date data lookups"],
    source: "https://docs.lovable.dev/integrations/perplexity",
  },
  {
    id: "firecrawl-connector",
    name: "Firecrawl Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Scrape, crawl, and extract website content via Firecrawl.",
    description: "Firecrawl scrapes, crawls, and extracts website content. Used to feed web data into Lovable apps and AI features.",
    capabilities: ["Website scraping", "Crawling and extraction"],
    useCases: ["Data ingestion for analytics", "Content extraction pipelines"],
    source: "https://docs.lovable.dev/integrations/firecrawl",
  },
  {
    id: "slack-app-connector",
    name: "Slack App Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "",
    tagline: "Send alerts, read channels, and post updates to Slack.",
    description: "Slack app connector lets Lovable apps send alerts, read channels, and post updates — distinct from the Slack MCP chat connector.",
    capabilities: ["Send messages and alerts", "Read channel content"],
    useCases: ["App notifications to internal channels", "Slack-based ops dashboards"],
    source: "https://docs.lovable.dev/integrations/slack",
  },
  {
    id: "twitch-connector",
    name: "Twitch Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Interact with Twitch streams from your app.",
    description: "Twitch connector lets apps interact with Twitch streams, chat, and metadata.",
    capabilities: ["Stream and chat interactions"],
    useCases: ["Streamer-facing dashboards"],
    source: "https://docs.lovable.dev/integrations/twitch",
  },
  {
    id: "twilio-connector",
    name: "Twilio Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Send SMS, MMS, and make voice calls via Twilio.",
    description: "Twilio connector enables SMS, MMS, and voice calls from Lovable apps.",
    capabilities: ["SMS and MMS sending", "Voice calls"],
    useCases: ["2FA codes", "Appointment reminders"],
    source: "https://docs.lovable.dev/integrations/twilio",
  },
  {
    id: "linear-app-connector",
    name: "Linear App Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Work with Linear issues from your app — distinct from the Linear MCP.",
    description: "Linear app connector lets apps create and update Linear issues. Separate from the Linear MCP chat connector which feeds issues into the agent.",
    capabilities: ["Create and update issues", "Read issue data"],
    useCases: ["Customer-feedback-to-issue flows"],
    source: "https://docs.lovable.dev/integrations/linear",
  },
  {
    id: "telegram-connector",
    name: "Telegram App Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Send messages and receive commands through Telegram bots.",
    description: "Telegram app connector lets apps send messages and receive commands through Telegram bots — distinct from the standalone Lovable Telegram Bot for building projects.",
    capabilities: ["Send messages", "Receive bot commands"],
    useCases: ["Customer-facing Telegram bots"],
    source: "https://docs.lovable.dev/integrations/telegram",
  },
  {
    id: "contentful-connector",
    name: "Contentful Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Fetch and manage Contentful CMS content.",
    description: "Contentful connector fetches and manages content from Contentful CMS for content-driven apps.",
    capabilities: ["Fetch published content", "Manage entries"],
    useCases: ["Marketing sites backed by Contentful"],
    source: "https://docs.lovable.dev/integrations/contentful",
  },
  {
    id: "aikido-connector",
    name: "Aikido Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "AI-powered penetration testing on your app.",
    description: "Aikido connector runs AI-powered penetration tests against the app to find exploitable vulnerabilities.",
    capabilities: ["AI-powered pentest", "Vulnerability detection"],
    useCases: ["Pre-launch security review"],
    source: "https://docs.lovable.dev/integrations/aikido",
  },
  {
    id: "aws-s3-connector",
    name: "AWS S3 Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Read and write files to AWS S3 buckets.",
    description: "AWS S3 connector enables read/write access to S3 buckets directly from app workflows.",
    capabilities: ["S3 read and write", "Bucket access"],
    useCases: ["Apps with existing S3 storage", "Hybrid storage architectures"],
    source: "https://docs.lovable.dev/integrations/aws-s3",
  },
  {
    id: "google-workspace-connector",
    name: "Google Workspace Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Gmail, Drive, Docs, Sheets, Slides, and Calendar access.",
    description: "Google Workspace productivity connector bundles Gmail, Drive, Docs, Sheets, Slides, and Calendar in one connection.",
    capabilities: ["Gmail, Drive, Docs, Sheets, Slides, Calendar", "Single OAuth connection"],
    useCases: ["Apps that read Drive documents", "Calendar-aware tools"],
    source: "https://docs.lovable.dev/integrations/google-workspace",
  },
  {
    id: "microsoft-365-connector",
    name: "Microsoft 365 Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Outlook, Teams, OneDrive, Word, Excel, PowerPoint, OneNote.",
    description: "Microsoft 365 productivity connector bundles Outlook, Teams, OneDrive, Word, Excel, PowerPoint, and OneNote in one connection.",
    capabilities: ["Outlook, Teams, OneDrive", "Word, Excel, PowerPoint, OneNote"],
    useCases: ["Enterprise apps integrating with M365", "Email and calendar automation"],
    source: "https://docs.lovable.dev/integrations/microsoft",
  },
  {
    id: "bigquery-connector",
    name: "BigQuery Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Query datasets and build analytics on Google BigQuery.",
    description: "BigQuery data warehouse connector enables querying datasets and building analytics dashboards on top of BigQuery.",
    capabilities: ["Query BigQuery datasets", "Analytics dashboards"],
    useCases: ["Internal BI tools", "Customer-facing analytics"],
    source: "https://docs.lovable.dev/integrations/bigquery",
  },
  {
    id: "databricks-connector",
    name: "Databricks Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Run SQL and call Databricks workspace APIs.",
    description: "Databricks connector runs SQL and calls Databricks workspace APIs from Lovable apps.",
    capabilities: ["SQL queries", "Workspace API calls"],
    useCases: ["Apps on top of Databricks data"],
    source: "https://docs.lovable.dev/integrations/databricks",
  },
  {
    id: "snowflake-connector",
    name: "Snowflake Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Query data and run SQL against Snowflake.",
    description: "Snowflake connector runs SQL and queries data warehouses against Snowflake.",
    capabilities: ["Snowflake SQL queries"],
    useCases: ["Customer-facing analytics dashboards"],
    source: "https://docs.lovable.dev/integrations/snowflake",
  },
  {
    id: "asana-connector",
    name: "Asana Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Read and write Asana projects, tasks, and assignments.",
    description: "Asana business connector enables apps to read and write Asana projects, tasks, and assignments.",
    capabilities: ["Read/write tasks and projects"],
    useCases: ["Internal apps that surface Asana work"],
    source: "https://docs.lovable.dev/integrations/asana",
  },
  {
    id: "ashby-connector",
    name: "Ashby Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Manage jobs, candidates, applications, and hiring workflows.",
    description: "Ashby ATS connector enables hiring-workflow apps to manage jobs, candidates, and applications.",
    capabilities: ["Job, candidate, application management"],
    useCases: ["Custom recruiting dashboards"],
    source: "https://docs.lovable.dev/integrations/ashby",
  },
  {
    id: "hubspot-connector",
    name: "HubSpot Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Work with HubSpot CRM contacts, companies, deals, and service data.",
    description: "HubSpot connector reads and writes CRM contacts, companies, deals, and service data.",
    capabilities: ["CRM contacts, companies, deals", "Service data"],
    useCases: ["Sales-team internal tools"],
    source: "https://docs.lovable.dev/integrations/hubspot",
  },
  {
    id: "inngest-connector",
    name: "Inngest Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Durable background jobs, schedules, and event-driven workflows.",
    description: "Inngest workflow connector runs durable background jobs, schedules, and event-driven workflows from Lovable apps.",
    capabilities: ["Durable background jobs", "Scheduled tasks", "Event-driven workflows"],
    useCases: ["Long-running data pipelines", "Reliable cron alternatives"],
    source: "https://docs.lovable.dev/integrations/inngest",
  },
  {
    id: "fireflies-connector",
    name: "Fireflies Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Read Fireflies meeting transcripts, summaries, and conversation intelligence.",
    description: "Fireflies connector reads meeting transcripts, summaries, and conversation intelligence into Lovable apps.",
    capabilities: ["Transcript and summary access", "Conversation intelligence"],
    useCases: ["Meeting-aware CRMs", "Sales coaching tools"],
    source: "https://docs.lovable.dev/integrations/fireflies",
  },
  {
    id: "gemini-enterprise-connector",
    name: "Gemini Enterprise Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Access Gemini Enterprise knowledge from inside your app.",
    description: "Gemini Enterprise connector pulls in enterprise Gemini knowledge as a source for Lovable apps.",
    capabilities: ["Gemini Enterprise knowledge access"],
    useCases: ["Enterprise knowledge-grounded apps"],
    source: "https://docs.lovable.dev/integrations/gemini-enterprise",
  },
  {
    id: "wordpress-connector",
    name: "WordPress.com Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Read and manage WordPress.com sites and content.",
    description: "WordPress.com connector reads and manages content on WordPress.com sites.",
    capabilities: ["Read and manage WP.com content"],
    useCases: ["Content dashboards over WP.com sites"],
    source: "https://docs.lovable.dev/integrations/wordpress-com",
  },
  {
    id: "airtable-connector",
    name: "Airtable Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Read and write Airtable bases, tables, and records.",
    description: "Airtable connector reads and writes bases, tables, and records — appears in the integrations index.",
    capabilities: ["Read/write Airtable records"],
    useCases: ["Airtable-backed internal tools"],
    source: "https://docs.lovable.dev/integrations/airtable",
  },
  {
    id: "attention-connector",
    name: "Attention Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Sales conversations, transcripts, and scorecards.",
    description: "Attention sales-intelligence connector accesses conversations, transcripts, and scorecards.",
    capabilities: ["Sales transcript and scorecard access"],
    useCases: ["Revenue ops dashboards"],
    source: "https://docs.lovable.dev/integrations/attention",
  },
  {
    id: "brevo-connector",
    name: "Brevo Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Transactional and marketing email via Brevo.",
    description: "Brevo connector sends transactional and marketing email and manages contacts.",
    capabilities: ["Transactional + marketing email", "Contact management"],
    useCases: ["Newsletter tools", "Notification email"],
    source: "https://docs.lovable.dev/integrations/brevo",
  },
  {
    id: "mailgun-connector",
    name: "Mailgun Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Send transactional email from a verified Mailgun domain.",
    description: "Mailgun connector sends transactional email from a verified domain.",
    capabilities: ["Transactional email sending"],
    useCases: ["Order confirmations", "Password resets"],
    source: "https://docs.lovable.dev/integrations/mailgun",
  },
  {
    id: "granola-connector",
    name: "Granola App Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Access AI-generated meeting notes from Granola.",
    description: "Granola app connector accesses meeting notes, summaries, and transcripts — distinct from the Granola MCP chat connector.",
    capabilities: ["Meeting notes and summaries"],
    useCases: ["Knowledge bases from meetings"],
    source: "https://docs.lovable.dev/integrations/granola",
  },
  {
    id: "notion-app-connector",
    name: "Notion App Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Read and write Notion pages and databases.",
    description: "Notion app connector reads and writes pages and queries databases — distinct from the Notion MCP chat connector.",
    capabilities: ["Read/write pages", "Query Notion databases"],
    useCases: ["Notion-backed apps", "Content migration"],
    source: "https://docs.lovable.dev/integrations/notion",
  },
  {
    id: "wiz-connector",
    name: "Wiz Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Scan project dependencies for known vulnerabilities.",
    description: "Wiz connector scans project dependencies for known vulnerabilities.",
    capabilities: ["Dependency vulnerability scanning"],
    useCases: ["Compliance-driven dependency audits"],
    source: "https://docs.lovable.dev/integrations/wiz",
  },
  {
    id: "storyblok-connector",
    name: "Storyblok Connector",
    category: "App Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Fetch published or preview content from Storyblok CDN.",
    description: "Storyblok connector fetches published or preview content from Storyblok's Content Delivery API.",
    capabilities: ["Content delivery API access"],
    useCases: ["Headless CMS sites"],
    source: "https://docs.lovable.dev/integrations/storyblok",
  },
  {
    id: "atlassian-mcp-connector",
    name: "Atlassian MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Jira issues and Confluence docs in the Lovable agent.",
    description: "Atlassian MCP connector lets the agent read Jira issues and Confluence documentation to ground prototype generation in real tickets and specs.",
    capabilities: ["Read Jira issues", "Read Confluence docs"],
    useCases: ["Building features from existing tickets", "Spec-driven prototypes"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "notion-mcp-connector",
    name: "Notion MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Reference Notion docs and pages in the agent.",
    description: "Notion MCP connector lets the agent reference docs and pages to guide app behavior — distinct from the Notion app connector for in-app integrations.",
    capabilities: ["Reference Notion pages", "Ground agent in Notion docs"],
    useCases: ["Spec-driven generation", "Wiki-aware prototyping"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "linear-mcp-connector",
    name: "Linear MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Import Linear issues and specs into the Lovable agent.",
    description: "Linear MCP connector imports issues and specs to generate features — distinct from the Linear app connector.",
    capabilities: ["Import Linear issues", "Generate features from specs"],
    useCases: ["Issue-driven prototyping"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "n8n-mcp-connector",
    name: "n8n MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Mirror or plug into existing n8n automation workflows.",
    description: "n8n MCP connector lets the agent mirror or plug into existing n8n automation workflows.",
    capabilities: ["Workflow context", "Automation integration"],
    useCases: ["Building UIs over n8n flows"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "miro-mcp-connector",
    name: "Miro MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2025-12-10",
    pricing: "All plans",
    icon: "",
    tagline: "Read Miro boards, wireframes, and diagrams.",
    description: "Miro MCP connector lets the agent access boards, wireframes, and diagrams to ground UI generation in design source material.",
    capabilities: ["Access boards and wireframes"],
    useCases: ["Wireframe-to-UI workflows"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "granola-mcp-connector",
    name: "Granola MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "",
    tagline: "Turn Granola meeting notes into specs for the agent.",
    description: "Granola MCP connector turns meeting notes, decisions, and action items into specs the agent can use.",
    capabilities: ["Meeting notes as spec input"],
    useCases: ["From a discovery call to a prototype"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "amplitude-mcp-connector",
    name: "Amplitude MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "",
    tagline: "Product analytics and user feedback into the agent.",
    description: "Amplitude MCP connector brings product analytics and user feedback into the agent to inform feature decisions.",
    capabilities: ["Analytics-grounded suggestions"],
    useCases: ["Data-informed feature planning"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "polar-mcp-connector",
    name: "Polar MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Billing, products, and subscription data for SaaS scaffolding.",
    description: "Polar MCP connector uses billing, products, and subscription data to scaffold SaaS apps.",
    capabilities: ["Billing and subscription data access"],
    useCases: ["SaaS scaffolding grounded in real plans"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "sanity-mcp-connector",
    name: "Sanity MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "All plans",
    icon: "",
    tagline: "Access Sanity CMS content and schemas.",
    description: "Sanity MCP connector accesses CMS content and structured schemas to drive content-aware prototypes.",
    capabilities: ["CMS content and schema access"],
    useCases: ["Headless-CMS-driven sites"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "hex-mcp-connector",
    name: "Hex MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Notebooks and analyses as grounding for app generation.",
    description: "Hex MCP connector uses notebooks and data workflows to build apps grounded in analyses.",
    capabilities: ["Notebook and workflow context"],
    useCases: ["Analytics-backed internal tools"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "confidence-mcp-connector",
    name: "Confidence MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Feature flags and experiment results in the agent.",
    description: "Confidence MCP connector evaluates feature flags and accesses experiment results for the agent.",
    capabilities: ["Feature flag and experiment context"],
    useCases: ["Experiment-aware app building"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "posthog-mcp-connector",
    name: "PostHog MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "PostHog product analytics inside the Lovable agent.",
    description: "PostHog MCP connector brings product analytics and user behavior insights into agent context.",
    capabilities: ["Analytics-driven feature suggestions"],
    useCases: ["Building features informed by usage data"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "sentry-mcp-connector",
    name: "Sentry MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "All plans",
    icon: "",
    tagline: "Error tracking and performance monitoring for the agent.",
    description: "Sentry MCP connector exposes error tracking and performance monitoring data to the agent.",
    capabilities: ["Error and performance context"],
    useCases: ["Bug fixing with full error context"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "heygen-mcp-connector",
    name: "HeyGen MCP Connector",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-04-15",
    pricing: "All plans",
    icon: "",
    tagline: "Generate AI avatars, voiceovers, and videos.",
    description: "HeyGen MCP connector generates AI avatars, voiceovers, and videos from inside the agent.",
    capabilities: ["AI avatars, voice, video"],
    useCases: ["In-app video generation"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "custom-mcp-servers",
    name: "Custom MCP Servers",
    category: "MCP Connectors",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "Paid plans",
    icon: "",
    tagline: "Bring your own MCP server — internal or third-party systems.",
    description: "Custom MCP Servers let paid-plan workspaces add their own MCP servers to connect internal or third-party systems to the agent.",
    capabilities: ["BYO MCP server", "Unauthenticated MCP support", "Tool permission controls"],
    useCases: ["Internal APIs available to the agent", "Vendor-specific tools"],
    source: "https://docs.lovable.dev/integrations/mcp-servers",
  },
  {
    id: "audit-logs",
    name: "Audit Logs",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Enterprise plans",
    icon: "",
    tagline: "Searchable record of workspace actions, retained ~90 days.",
    description: "Audit Logs track membership, workspace management, groups, identity and access, secrets and integrations, projects, and authentication events. Retained ~13 weeks. Export to JSONL added April 2026 for Enterprise.",
    capabilities: ["Track membership, identity, integrations, projects, auth", "~90 day retention", "JSONL export"],
    useCases: ["Compliance audits", "Investigating anomalies"],
    source: "https://docs.lovable.dev/features/audit-logs",
  },
  {
    id: "security-center",
    name: "Security Center",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Workspace-wide dashboard for code scans, dependencies, secrets, auth policy.",
    description: "Security Center unifies four scan domains (code, supply chain, secrets, authentication policy) across all workspace projects. Includes CSV export, scan timestamps, and filtering by severity/status/project visibility. Auto-runs scans before publishing.",
    capabilities: ["Code, dependency, secrets, auth-policy scanning", "CSV export", "Pre-publish gating on critical findings"],
    useCases: ["Workspace-wide security posture", "Compliance reporting"],
    source: "https://docs.lovable.dev/features/security-center",
  },
  {
    id: "security-view",
    name: "Security View (Per-Project)",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2025-06-04",
    pricing: "All plans",
    icon: "",
    tagline: "Per-project security review with RLS, code, and dependency scans.",
    description: "Security View runs RLS analysis, database security checks, code security review, and npm dependency audits per project. Scans trigger automatically on file changes and before publishing, and can be run on-demand.",
    capabilities: ["RLS analysis", "Code security review", "npm dependency audit", "Pre-publish blocking"],
    useCases: ["Pre-launch hardening", "Catching RLS misconfigurations"],
    source: "https://docs.lovable.dev/features/security",
  },
  {
    id: "workspace-sso",
    name: "Workspace SSO (OIDC + SAML 2.0)",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2025-08-11",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Sign in to Lovable workspaces via Okta, Auth0, Entra ID, and more.",
    description: "OIDC and SAML 2.0 SSO for workspace access. Supports any compliant IdP, JIT provisioning with configurable default roles, and enforceable session durations (24-48h or 7 days). Service-provider initiated only.",
    capabilities: ["OIDC and SAML 2.0", "JIT provisioning", "Configurable session durations", "SSO enforcement"],
    useCases: ["Enterprise identity centralization", "Compliance-mandated auth"],
    source: "https://docs.lovable.dev/features/business/sso",
  },
  {
    id: "scim-provisioning",
    name: "SCIM Provisioning",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "Enterprise plans",
    icon: "",
    tagline: "Automated user provisioning and lifecycle via Okta or Entra ID.",
    description: "SCIM 2.0 provisioning automates user creation, role assignment, and deprovisioning through Okta, Microsoft Entra ID, or any compliant provider. Highest-privilege role wins when groups overlap. Requires active SSO.",
    capabilities: ["SCIM 2.0 with Okta, Entra ID", "Auto-provision and deprovision", "Group-based role mapping", "Activity overview"],
    useCases: ["Lifecycle automation for large orgs", "Group-driven access control"],
    source: "https://docs.lovable.dev/features/business/scim",
  },
  {
    id: "two-factor-authentication",
    name: "Two-Factor Authentication (2FA)",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "",
    tagline: "Authenticator app or SMS 2FA on every Lovable account.",
    description: "Two-factor authentication via TOTP authenticator app (Google Authenticator, Microsoft Authenticator, Authy, 1Password) or SMS. Works across all sign-in methods (email, Google, GitHub, Apple, SSO). Multi-method recommended to prevent lockout.",
    capabilities: ["TOTP authenticator app", "SMS codes", "Multi-method enrollment"],
    useCases: ["Account hardening", "Shared workspace protection"],
    source: "https://docs.lovable.dev/introduction/two-factor-authentication-2-fa",
  },
  {
    id: "data-opt-out",
    name: "Data Training Opt-Out",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2025-09-01",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Self-serve opt-out from AI model training for workspace data.",
    description: "Workspace admins on Business and Enterprise plans can self-serve disable use of workspace data for model training. Free and Pro plans must request opt-out through Lovable Support. Lovable commits to never using identifiable PII for training.",
    capabilities: ["Self-serve workspace opt-out (Business/Enterprise)", "Support-driven opt-out (Free/Pro)"],
    useCases: ["Compliance with internal data policies", "Protecting proprietary code"],
    source: "https://docs.lovable.dev/features/business/data-opt-out",
  },
  {
    id: "auth-policy",
    name: "Workspace Authentication Policy",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Configure which sign-in methods (email, Google, Apple, SAML) are allowed.",
    description: "Workspace Authentication Policy controls which sign-in methods (email, phone, Google, Apple, SAML) are permitted across the workspace. Surfaced in Security Center.",
    capabilities: ["Method-level allow/deny", "SAML enforcement"],
    useCases: ["Forcing SSO-only org sign-in"],
    source: "https://docs.lovable.dev/features/security-center",
  },
  {
    id: "dependency-vulnerability-scanning",
    name: "Dependency Vulnerability Scanning",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "",
    tagline: "Auto-detect npm dependency vulnerabilities by severity.",
    description: "Automatic supply-chain scanning ranks dependency vulnerabilities by severity (critical, high, medium), shows which packages are affected, and surfaces available fixes.",
    capabilities: ["Severity ranking", "Affected-package visibility", "Fix suggestions"],
    useCases: ["Pre-launch dependency hygiene"],
    source: "https://docs.lovable.dev/features/security-center",
  },
  {
    id: "export-secrets-csv",
    name: "Export Secrets (CSV)",
    category: "Security",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Export secret inventory as CSV for audits (names only, no values).",
    description: "Export Secrets exports the workspace's secret inventory as CSV — names and project usage only, values never leave Lovable.",
    capabilities: ["CSV export of secret names", "Project usage mapping"],
    useCases: ["Compliance audits", "Secret inventory reviews"],
    source: "https://docs.lovable.dev/features/security-center",
  },
  {
    id: "groups",
    name: "Groups",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Team-based access controls with SCIM sync.",
    description: "Groups organize workspace members for project sharing, folder sharing, and published-app visibility. Sync from identity providers via SCIM with a SCIM badge. Higher of individual vs group permission applies. Audit-logged on Enterprise.",
    capabilities: ["Group-based project and folder sharing", "Published-app visibility", "SCIM sync"],
    useCases: ["Granting whole teams access at once", "Restricting published apps to a department"],
    source: "https://docs.lovable.dev/features/groups",
  },
  {
    id: "project-knowledge",
    name: "Project Knowledge",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-04-25",
    pricing: "All plans",
    icon: "",
    tagline: "Project-specific agent context — schema, terminology, persona (10K chars).",
    description: "Project Knowledge stores app purpose, schema, domain terminology, and architectural decisions for a single project. Any editor can update. Wins over workspace knowledge on conflict. Up to 10,000 characters.",
    capabilities: ["Project-specific persistent context", "10,000-character capacity", "Wins over workspace knowledge on conflict"],
    useCases: ["Onboarding the agent to a domain", "Schema and terminology stability"],
    source: "https://docs.lovable.dev/features/knowledge",
  },
  {
    id: "branded-workspace-urls",
    name: "Branded Workspace URLs",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2025-08-11",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Publish apps at app-name.workspace.lovable.app.",
    description: "Branded Workspace URLs publish workspace apps under a consistent subdomain pattern derived from a verified domain. Coexists with custom domains (custom domains take precedence). Preview URLs use preview--{app}.{workspace}.lovable.app.",
    capabilities: ["Unified workspace URL pattern", "Auto-derived from verified domain", "Preview URL support"],
    useCases: ["Internal app portals", "Branded staging links"],
    source: "https://docs.lovable.dev/features/branded-workspace-urls",
  },
  {
    id: "custom-domain",
    name: "Custom Domain",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2025-03-21",
    pricing: "Paid plans",
    icon: "",
    tagline: "Connect your domain — automatic SSL, multi-subdomain, primary selection.",
    description: "Custom Domain replaces the default lovable.app URL with your branded domain. Includes automatic SSL/HTTPS provisioning, multi-subdomain support, primary domain selection, and optional Cloudflare/CloudFront/Fastly proxy mode (you manage the CDN).",
    capabilities: ["Automatic SSL provisioning", "Multiple subdomains per workspace", "Primary domain selection", "CDN/reverse-proxy mode"],
    useCases: ["Production deployments on your brand", "Multi-app subdomain layouts"],
    source: "https://docs.lovable.dev/features/custom-domain",
  },
  {
    id: "ionos-domain-purchasing",
    name: "Domain Purchasing (IONOS)",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2025-04-11",
    pricing: "Paid plans",
    icon: "",
    tagline: "Buy a domain inside Lovable via IONOS partnership.",
    description: "IONOS partnership lets users purchase a domain directly inside Lovable. DNS and SSL are configured automatically. Supports major gTLDs and selected ccTLDs (.ai, .io, .co, .me). Redesigned April 2026.",
    capabilities: ["In-product domain purchase via IONOS", "Automatic DNS and SSL", "Major gTLD + select ccTLD support"],
    useCases: ["Buying a domain at launch without leaving Lovable"],
    source: "https://docs.lovable.dev/features/custom-domain",
  },
  {
    id: "cdn-reverse-proxy",
    name: "CDN and Reverse Proxy Support",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Paid plans",
    icon: "",
    tagline: "Front your Lovable app with Cloudflare, CloudFront, or Fastly.",
    description: "CDN and reverse-proxy mode lets you front a Lovable-published app with Cloudflare, CloudFront, or Fastly. Removed Feb 23, 2026 and reintroduced April 2, 2026. Lovable does not configure or monitor the proxy — that's on you.",
    capabilities: ["Cloudflare, CloudFront, Fastly support", "Self-managed proxy configuration"],
    useCases: ["Edge caching", "WAF in front of Lovable"],
    source: "https://docs.lovable.dev/features/custom-domain",
  },
  {
    id: "custom-app-emails",
    name: "Custom App Emails",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Lovable Cloud (paid)",
    icon: "",
    tagline: "Branded transactional emails — 50K/month included on paid Cloud workspaces.",
    description: "App emails are transactional emails (order confirmations, shipping updates, account notifications) auto-branded from your app's CSS variables, fonts, and logo. 50,000 emails per month included; $1 per 1,000 above that. Mandatory unsubscribe footer.",
    capabilities: ["Auto-branded templates from app CSS and logo", "50K/month included", "Suppression lists"],
    useCases: ["Order and shipping notifications", "Account event emails"],
    source: "https://docs.lovable.dev/features/custom-emails",
  },
  {
    id: "authentication-emails-custom-domain",
    name: "Authentication Emails from Custom Domain",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2026-03-16",
    pricing: "Lovable Cloud (paid)",
    icon: "",
    tagline: "Send auth emails from your domain with auto-managed DNS.",
    description: "Authentication emails (password reset, email verification) ship from your custom domain with automatic DNS management. Email rate-limit configuration controls send velocity.",
    capabilities: ["Auth emails from custom domain", "Automatic DNS management", "Rate-limit configuration"],
    useCases: ["Deliverability for transactional auth", "Brand-consistent magic-link emails"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "granular-publishing",
    name: "Granular Publishing Access",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Restrict published-app access to specific people or groups.",
    description: "Granular Publishing restricts published-app access to specific workspace members or groups rather than the entire workspace or public. Enterprise can additionally restrict who can trigger publishes.",
    capabilities: ["Member- and group-level access", "Per-app restriction lists", "Enterprise: restrict who can publish"],
    useCases: ["Internal-only apps for one team", "Tight launch control"],
    source: "https://docs.lovable.dev/features/publish",
  },
  {
    id: "publish-snapshot-deployment",
    name: "Publish (Snapshot Deployment)",
    category: "Publishing",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans",
    icon: "",
    tagline: "Deploy project snapshots to live URLs with metadata controls.",
    description: "Publish deploys a project snapshot to a live shareable URL with favicon, title, meta description, and social sharing image configuration. Changes require explicit republishing. Security scans gate publishes.",
    capabilities: ["Snapshot deployment to live URL", "Metadata controls (favicon, title, OG image)", "Security scans before publish"],
    useCases: ["Shipping a project publicly", "Updating a live app on demand"],
    source: "https://docs.lovable.dev/features/publish",
  },
  {
    id: "share-project",
    name: "Share Project",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans (Viewer/restricted: paid plans)",
    icon: "",
    tagline: "Email invites, 5-day invite links, 7-day public preview links.",
    description: "Share Project supports email invitations with role assignment, 5-day invite links, and 7-day public preview links. Viewer role requires paid plans. Business and Enterprise add restricted projects and SSO enforcement.",
    capabilities: ["Email invites with roles", "Invite links (5-day) and preview links (7-day)", "Viewer role on paid plans"],
    useCases: ["External collaborator access", "Stakeholder previews without accounts"],
    source: "https://docs.lovable.dev/features/share-project",
  },
  {
    id: "workspace-discovery",
    name: "Workspace Discovery (Domain JIT)",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Auto-discover workspaces by verified company email domain.",
    description: "Workspace Discovery auto-routes users with verified company email domains into the matching workspace. Bulk Domain JIT provisioning shipped April 2026.",
    capabilities: ["Domain-verified discovery", "Bulk JIT provisioning"],
    useCases: ["Self-serve workspace joins at companies"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "project-ownership-transfer",
    name: "Project Ownership Transfer",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-02-23",
    pricing: "All plans",
    icon: "",
    tagline: "Transfer projects between workspaces with editor permissions.",
    description: "Project Ownership Transfer moves a project between workspaces. Editors can transfer projects, with name choice and dropdown access from project cards.",
    capabilities: ["Cross-workspace transfer", "Editor-permitted transfer"],
    useCases: ["Moving a freelance project to a client workspace"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "free-plan",
    name: "Free Plan",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "Free",
    icon: "",
    tagline: "5 daily credits (30/month cap), unlimited members, private projects.",
    description: "The Free plan provides 5 daily credits (capped at 30/month), unlimited workspace members, and private projects. Daily credits do not roll over.",
    capabilities: ["5 daily credits (30/month cap)", "Unlimited members", "Private projects"],
    useCases: ["Hobby builds", "Trial usage"],
    source: "https://docs.lovable.dev/introduction/plans-and-credits",
  },
  {
    id: "pro-plan",
    name: "Pro Plan",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "$25-$2,250/month or $250-$22,500/year",
    icon: "",
    tagline: "100-10,000 credits/month, custom domains, code mode, badge removal.",
    description: "Pro plan offers 100 to 10,000 monthly credits, workspace roles, custom domains, Code mode access, and 'Edit with Lovable' badge removal. Annual billing saves 20%.",
    capabilities: ["100-10,000 monthly credits", "Custom domains", "Code mode", "Badge removal"],
    useCases: ["Solo builders and freelancers", "Small production apps"],
    source: "https://docs.lovable.dev/introduction/plans-and-credits",
  },
  {
    id: "business-plan",
    name: "Business Plan",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-08-11",
    pricing: "$50-$4,300/month or $500-$43,000/year",
    icon: "",
    tagline: "SSO, restricted projects, data opt-out, design templates.",
    description: "Business plan adds SSO (OIDC + SAML), restricted project access, data training opt-out, design template creation, and granular publishing controls on top of Pro.",
    capabilities: ["SSO (OIDC + SAML)", "Restricted projects", "Data opt-out", "Design templates"],
    useCases: ["Mid-sized teams", "Companies requiring SSO"],
    source: "https://docs.lovable.dev/introduction/plans-and-credits",
  },
  {
    id: "enterprise-plan",
    name: "Enterprise Plan",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "Custom",
    icon: "",
    tagline: "SCIM, audit logs, code execution, SSO enforcement, per-member credit limits.",
    description: "Enterprise plan adds SCIM provisioning, audit logs with JSONL export, SSO-only enforcement, per-member credit limits, GitHub Enterprise (Cloud and Server), and code execution. Custom-priced.",
    capabilities: ["SCIM provisioning", "Audit logs and JSONL export", "GitHub Enterprise Cloud/Server", "SSO enforcement"],
    useCases: ["Regulated industries", "Large orgs needing IdP-driven lifecycle"],
    source: "https://docs.lovable.dev/introduction/plans-and-credits",
  },
  {
    id: "lovable-bonuses",
    name: "Lovable Bonuses (Daily Streak Credits)",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2026-01-16",
    pricing: "All plans",
    icon: "",
    tagline: "Daily 5-credit bonus for hitting 25 messages.",
    description: "Lovable Bonuses awards a daily 5-credit bonus for sending 25 messages, rewarding active building.",
    capabilities: ["Daily 5-credit bonus", "25-message threshold"],
    useCases: ["Active daily builders"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "gift-cards",
    name: "Lovable Gift Cards",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Gift Lovable credits or subscriptions.",
    description: "Lovable Gift Cards let users gift credits or subscriptions to other Lovable users.",
    capabilities: ["Gift credits or subscriptions"],
    useCases: ["Gifting builds to friends", "Education incentives"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "education-logins",
    name: "Education Logins",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "Education",
    icon: "",
    tagline: "Temporary education login flow for classrooms.",
    description: "Education Logins provide a temporary, lightweight login experience for classroom and education contexts.",
    capabilities: ["Temporary classroom logins"],
    useCases: ["Workshop and classroom usage"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "linkedin-vibe-coding-levels",
    name: "LinkedIn Vibe Coding Levels",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "All plans",
    icon: "",
    tagline: "Bronze through Diamond certification tracked from real building.",
    description: "Five vibe-coding tiers (Bronze, Silver, Gold, Platinum, Diamond) tracked automatically by activity and surfaced on LinkedIn under Licenses & Certifications. Non-expiring, real-time, opt-in to display.",
    capabilities: ["Five tiers (Bronze, Silver, Gold, Platinum, Diamond)", "Activity-based progression", "Non-expiring"],
    useCases: ["Public credentials for builders", "Hiring signals"],
    source: "https://docs.lovable.dev/tips-tricks/linkedin-certification",
  },
  {
    id: "lovable-chatgpt-app",
    name: "Lovable ChatGPT App",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "All plans",
    icon: "",
    tagline: "Tag @Lovable in ChatGPT to turn brainstorms into full-stack apps.",
    description: "The Lovable ChatGPT App lets users tag @Lovable inside ChatGPT to convert conversations into Lovable projects. Initial build happens in ChatGPT; iteration moves to Lovable. Credits only charge when the build kicks off.",
    capabilities: ["Tag @Lovable in ChatGPT", "Build from conversation context", "Account connection flow"],
    useCases: ["Idea-to-prototype in one tool", "Non-coder app starts"],
    source: "https://docs.lovable.dev/tips-tricks/chatgpt-app",
  },
  {
    id: "lovable-telegram-bot",
    name: "Lovable Telegram Bot",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2026-01-01",
    pricing: "Free and Pro workspaces",
    icon: "",
    tagline: "Build and manage Lovable projects from Telegram DMs.",
    description: "The Lovable Telegram Bot lets users build, modify, fix, publish, and analyze projects via natural-language Telegram DMs. Read-only DB access, no group chats, one Telegram account per Lovable user. Available on Free and Pro workspaces.",
    capabilities: ["Build, modify, publish from DMs", "Read-only DB queries", "Cross-project search", "Web search"],
    useCases: ["Mobile-first iteration", "Off-laptop bug fixes"],
    source: "https://docs.lovable.dev/tips-tricks/lovable-telegram-bot",
  },
  {
    id: "lovable-api",
    name: "Lovable API",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans",
    icon: "",
    tagline: "Programmatic project creation, starting with Build with URL.",
    description: "The Lovable API enables programmatic project creation and distribution. First-class endpoint is Build with URL; more endpoints are landing over time. Powers Build-with-Lovable buttons and link-driven workflows.",
    capabilities: ["Build with URL", "Programmatic project creation", "Embeddable buttons"],
    useCases: ["Templated app launches", "Embedding Lovable in other products"],
    source: "https://docs.lovable.dev/integrations/lovable-api",
  },
  {
    id: "build-with-url",
    name: "Build with URL",
    category: "Integrations",
    status: "GA" as const,
    releaseDate: "2025-01-01",
    pricing: "All plans",
    icon: "",
    tagline: "Create Lovable apps from a URL with a prompt and up to 10 images.",
    description: "Build with URL constructs a Lovable project from a shareable URL containing a prompt and optionally up to 10 reference images. The prompt and images persist through signup if the user is new.",
    capabilities: ["URL-based app creation", "Up to 10 reference images", "Persist prompt through signup"],
    useCases: ["'Build with Lovable' buttons", "Bulk-generation from templates"],
    source: "https://docs.lovable.dev/integrations/build-with-url",
  },
  {
    id: "lovable-mcp-server-api",
    name: "Lovable MCP Server (API)",
    category: "MCP Connectors",
    status: "Beta" as const,
    releaseDate: "2025-01-01",
    pricing: "Pro and Business plans",
    icon: "",
    tagline: "Research-preview MCP server for managing Lovable from external AI clients.",
    description: "The Lovable MCP Server (research preview) exposes project creation, agent messaging, code inspection, Cloud Postgres queries, analytics, and workspace governance to MCP-compatible clients (Claude Desktop, claude.ai, Cursor, Windsurf, Claude Code). OAuth, not API keys. Tool shapes may change.",
    capabilities: ["Create/iterate/deploy projects", "Read files, diffs, edit history", "Query Cloud Postgres", "Workspace AI instructions"],
    useCases: ["Driving Lovable from Cursor", "Automated workflows from Claude Desktop"],
    source: "https://docs.lovable.dev/integrations/lovable-mcp-server",
  },
  {
    id: "analytics",
    name: "Real-Time App Analytics",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-08-11",
    pricing: "All plans",
    icon: "",
    tagline: "Visitors, pageviews, bounce rate, sources, and devices in real time.",
    description: "Real-Time App Analytics tracks visitors, pageviews, bounce rate, visit duration, traffic sources, and device usage for published apps. Time ranges from today through 90 days plus monthly views.",
    capabilities: ["Visitors, pageviews, bounce rate", "Traffic sources and devices", "Real-time updates"],
    useCases: ["Launch-day traffic monitoring", "Source-of-truth public analytics"],
    source: "https://docs.lovable.dev/features/analytics",
  },
  {
    id: "ai-powered-visual-edits",
    name: "AI-Powered Visual Edits",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans (with daily free limits)",
    icon: "",
    tagline: "Multi-element visual editing with AI image generation and text editing.",
    description: "Visual Edits v2 supports multi-selection, text/color/font edits, individual margin/padding, image replacement, and AI image generation inline. Free up to 100 edits/user/24h and 500/IP/24h; additional edits use credits.",
    capabilities: ["Multi-element selection", "AI image generation in-place", "Text, color, font, layout edits"],
    useCases: ["Designer-friendly tweaks", "Marketer copy edits"],
    source: "https://docs.lovable.dev/features/design",
  },
  {
    id: "mermaid-diagrams",
    name: "Mermaid Diagrams in Chat",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-06-04",
    pricing: "All plans",
    icon: "",
    tagline: "Chat renders Mermaid diagrams for architectures and flows.",
    description: "Chat mode renders Mermaid diagrams inline so the agent can show architectures, sequence flows, and data models without external tools.",
    capabilities: ["Inline Mermaid rendering"],
    useCases: ["Architectural reviews in chat"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "image-understanding",
    name: "Image Understanding (Multimodal Input)",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2025-05-09",
    pricing: "All plans",
    icon: "",
    tagline: "Attach images for the agent to interpret as design context.",
    description: "Image Understanding lets users attach images (mockups, screenshots, wireframes) and have the agent interpret them as guidance for implementation.",
    capabilities: ["Image-as-input for builds"],
    useCases: ["Designs to working UI"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "design-templates",
    name: "Design Templates",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-02-05",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Reusable project blueprints workspace-wide.",
    description: "Design Templates copy an entire project codebase as the foundation for new projects in the same workspace. Workspace admins can set one default template. Requires public or workspace project visibility.",
    capabilities: ["Full codebase copy as starting point", "Default template per workspace"],
    useCases: ["Brand-consistent app starts", "Internal architectural baselines"],
    source: "https://docs.lovable.dev/features/business/design-templates",
  },
  {
    id: "starred-projects-folders",
    name: "Starred Projects and Folders",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-12-10",
    pricing: "All plans",
    icon: "",
    tagline: "Star projects, nest folders, drag-drop organization.",
    description: "Dashboard supports starred projects, nested folders, drag-drop reorganization, persistent filters, server-side search across folders, and recently-viewed sorting.",
    capabilities: ["Starring", "Nested folders with drag-drop", "Server-side search across folders"],
    useCases: ["Organizing dozens of projects"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "folder-collaborators",
    name: "Folder Collaborators",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2026-04-24",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Share folders with groups or people — grants access to all projects inside.",
    description: "Folder Collaborators grants group or individual access to all projects within a folder, including folder visibility controls.",
    capabilities: ["Folder-level access controls", "Group or individual sharing"],
    useCases: ["Team-level project bundles"],
    source: "https://docs.lovable.dev/features/groups",
  },
  {
    id: "global-search",
    name: "Global Project Search",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-06-26",
    pricing: "All plans",
    icon: "",
    tagline: "Fuzzy search across projects, files, and folders.",
    description: "Global Project Search supports fuzzy matching, combined filename + content search, command palette (cmd+K) entry, and rich folder results.",
    capabilities: ["Fuzzy search", "Filename + content combined search", "Command palette integration"],
    useCases: ["Finding a project by partial name", "Code search across the workspace"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "command-palette",
    name: "Command Palette (Cmd+K)",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Cmd+K palette redesigned for navigation, search, and connectors.",
    description: "The Command Palette (Cmd+K) supports project search, rich folder results, and connector access that stays scoped to the current project.",
    capabilities: ["Project + folder search", "Connector access in-project"],
    useCases: ["Keyboard-driven navigation"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "image-tools-zoom-crop",
    name: "Image Tools (Zoom and Crop)",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-04-02",
    pricing: "All plans",
    icon: "",
    tagline: "Zoom and crop tools for in-product image editing.",
    description: "In-product image tools let users zoom and crop images during visual editing.",
    capabilities: ["Zoom and crop"],
    useCases: ["Trimming uploaded photos in-place"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "ionos-domain-discovery",
    name: "Verified Domain Workspace Discovery",
    category: "Workspace",
    status: "GA" as const,
    releaseDate: "2025-12-23",
    pricing: "Business and Enterprise plans",
    icon: "",
    tagline: "Verified domains help users find their workspace at signup.",
    description: "Verified domain workspace discovery routes signups from a company email domain into the matching workspace.",
    capabilities: ["Email-domain-based workspace routing"],
    useCases: ["Self-serve company joins"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "edge-function-logging",
    name: "Edge Function Logging",
    category: "Cloud",
    status: "GA" as const,
    releaseDate: "2025-03-28",
    pricing: "All plans",
    icon: "",
    tagline: "Inline logs for Edge Functions runs.",
    description: "Edge Function Logging surfaces serverless run logs inside the Lovable UI for debugging custom server-side code.",
    capabilities: ["In-product Edge Function logs"],
    useCases: ["Debugging Stripe webhooks"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "status-page",
    name: "Lovable Status Page",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-03-28",
    pricing: "All plans",
    icon: "",
    tagline: "Public uptime and incident status page.",
    description: "The Lovable Status Page surfaces uptime and incident updates for the platform.",
    capabilities: ["Real-time platform status"],
    useCases: ["Checking during a suspected outage"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "questions-feature",
    name: "Questions (Agent Clarification Tool)",
    category: "Agent",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Agent asks structured clarifying questions before building.",
    description: "The Questions feature lets the agent ask structured clarifying questions before starting work, captured separately from chat for explicit answers.",
    capabilities: ["Structured agent clarification"],
    useCases: ["Reducing wasted credits on ambiguous prompts"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "low-credit-alert",
    name: "Low Credit Alert",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "In-product alert when credits run low.",
    description: "Low Credit Alerts notify users before credits run out, with rollover and top-up reminders surfaced alongside.",
    capabilities: ["Low-balance notifications", "Top-up prompts"],
    useCases: ["Avoiding mid-build credit exhaustion"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "remix-experience",
    name: "Remix Experience",
    category: "Workflow",
    status: "GA" as const,
    releaseDate: "2025-11-26",
    pricing: "All plans",
    icon: "",
    tagline: "Remix any project — choose workspace, folder, and new name.",
    description: "The Remix Experience copies any public/community project into the user's workspace, with workspace + folder choice and a custom name during remix.",
    capabilities: ["Workspace/folder choice on remix", "Custom name during remix"],
    useCases: ["Starting from a community template"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "community-discovery",
    name: "Community Discovery and Categorization",
    category: "Community",
    status: "GA" as const,
    releaseDate: "2025-12-10",
    pricing: "All plans",
    icon: "",
    tagline: "Browse community projects by category and themes.",
    description: "Community Discovery surfaces projects shared by other builders, organized into categories with theming options for browsing.",
    capabilities: ["Categorized community feed"],
    useCases: ["Finding templates and inspiration"],
    source: "https://docs.lovable.dev/changelog",
  },
  {
    id: "design-guidance",
    name: "Design Guidance",
    category: "Editor",
    status: "GA" as const,
    releaseDate: "2026-01-15",
    pricing: "All plans",
    icon: "",
    tagline: "Preview three parallel design directions before building, then refine the one you pick.",
    description: "Design Guidance generates three parallel visual design directions for a new project — each rendered as HTML/Tailwind previews you can compare side-by-side. Guided design questions cover typography pairings, curated color palettes, and layout wireframes. Pick a direction and refine it up to six times with suggested prompts or custom text, or generate variations for individual sections like hero, navbar, pricing cards, and footers. The agent auto-skips Design Guidance when the visual direction is already clear from the prompt.",
    capabilities: ["Three parallel design direction previews in HTML/Tailwind", "Guided typography, color, and layout choices", "Refine selected direction up to six times", "Section-level variations (hero, navbar, pricing, footer)", "Auto-skip when direction is already clear"],
    useCases: ["Defining visual identity for a new landing page", "Exploring directions for a portfolio or blog", "Redesigning components in an existing project"],
    source: "https://docs.lovable.dev/features/design-guidance",
  },
  { id: "wiz-findings-integration", name: "Wiz Findings in Lovable", category: "Security", status: "GA" as const, releaseDate: "2026-05-07", pricing: "Business and Enterprise plans", icon: "", tagline: "Native Wiz security scans inside Lovable's Security view.", description: "Wiz security scanning runs automatically alongside Lovable's built-in checks. Findings map to existing Wiz policies and flow back into Wiz's Code and Build scans page, so organization-wide security standards apply to every Lovable project without separate tooling. Distinct from the basic Wiz connector — this is the tighter Findings integration.", capabilities: ["Software composition analysis","Secrets detection","Environment configuration scanning","Wiz policy mapping","Centralized Wiz dashboard sync"], useCases: ["Enterprise security governance","SOC 2 compliance posture","Org-wide policy enforcement"], source: "https://lovable.dev/blog/wiz-findings-now-in-lovable" },
  { id: "beyond-apps", name: "Beyond Apps (Data, Docs, Media)", category: "Workflow", status: "GA" as const, releaseDate: "2026-03-19", pricing: "Included with paid plans", icon: "", tagline: "Lovable now runs data analysis, generates docs, and creates media — not just apps.", description: "Lovable expanded past app building to handle data analysis (CSV, PDF, Amplitude metrics), document generation (PowerPoint, Word, PDF, Excel reports, investor decks, invoices), file editing across formats, and text-to-image/video media generation. The agent runs code and executes scripts in a secure sandbox to process files inline with app-building.", capabilities: ["CSV/PDF data analysis","PowerPoint/Word/Excel/PDF generation","Image generation","Video generation","Inline code execution in sandbox"], useCases: ["Marketing reports","Investor decks","Changelogs","Invoices","Data dashboards from CSVs"], source: "https://lovable.dev/blog/go-beyond-building-full-stack-apps-with-lovable" },
  { id: "prompt-queuing", name: "Prompt Queuing", category: "Workflow", status: "GA" as const, releaseDate: "2026-01-28", pricing: "All paid plans", icon: "", tagline: "Stack tasks in the background and reprioritize on the fly.", description: "Prompt queuing lets users stack multiple tasks for the agent to execute sequentially in the background. Queued prompts can be reordered or reprioritized while the agent works, enabling fire-and-forget batching of changes rather than waiting between each turn.", capabilities: ["Background task execution","Live reprioritization","Stacked multi-step builds"], useCases: ["Long build sessions","Background QA + fixes","Working async with the agent"], source: "https://lovable.dev/blog/a-smarter-lovable" },
  { id: "internal-publish", name: "Internal Publish", category: "Publishing", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Business and Enterprise plans", icon: "", tagline: "Publish apps internally to your workspace, not the public web.", description: "Internal Publish lets Business and Enterprise workspaces ship apps to an internal audience (org-only access) rather than the public lovable.app domain. Distinct from custom domains — this is access-controlled internal distribution, useful for internal tools.", capabilities: ["Workspace-scoped publishing","Org-only access","Separate from public publishing"], useCases: ["Internal tools","HR/finance dashboards","Private prototypes for stakeholders"], source: "https://lovable.dev/pricing" },
  { id: "user-roles-permissions", name: "User Roles and Permissions", category: "Workspace", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Pro, Business, Enterprise plans", icon: "", tagline: "Assign roles and permissions to workspace members.", description: "Workspace-level user roles and permissions controlling who can build, edit, publish, or manage projects. Available from Pro tier upward. Distinct from SSO/SCIM (Business/Enterprise only) which handles identity, not authorization scopes.", capabilities: ["Role assignment","Permission scoping","Member management"], useCases: ["Team collaboration","Limiting publish rights","Agency client management"], source: "https://lovable.dev/pricing" },
  { id: "design-systems-enterprise", name: "Design Systems (Enterprise)", category: "Editor", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Enterprise plans", icon: "", tagline: "Enterprise-grade reusable design systems across all org projects.", description: "Enterprise-tier feature distinct from per-project Themes — a centralized, org-wide design system applied across every workspace project. Couples with Themes but managed at org level.", capabilities: ["Org-wide design system","Cross-project component reuse","Centralized brand enforcement"], useCases: ["Enterprise brand governance","Cross-team design consistency"], source: "https://lovable.dev/pricing" },
  { id: "custom-connectors-enterprise", name: "Custom Connectors (Enterprise)", category: "MCP Connectors", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Enterprise plans", icon: "", tagline: "Build and deploy your own connectors beyond Lovable's catalog.", description: "Enterprise feature allowing organizations to author and deploy private connectors that aren't part of Lovable's public integration catalog. Distinct from public MCP integrations because it implies a customer-authored extension surface.", capabilities: ["Custom integration authoring","Private connector deployment","Internal API bridging"], useCases: ["Connect proprietary internal APIs","Industry-specific integrations"], source: "https://lovable.dev/pricing" },
  { id: "publishing-controls", name: "Publishing Controls", category: "Publishing", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Enterprise plans", icon: "", tagline: "Admin policy controls over who can publish what, where.", description: "Enterprise-only governance feature controlling publishing behavior across the org — who can ship to which audience (public, internal, custom domain), and approval flows. Distinct from per-user roles.", capabilities: ["Publish policy enforcement","Approval workflows","Audience restrictions"], useCases: ["Regulated industries","Pre-publish approval gates","Brand-safety policies"], source: "https://lovable.dev/pricing" },
  { id: "sharing-controls", name: "Sharing Controls", category: "Security", status: "GA" as const, releaseDate: "2025-08-11", pricing: "Enterprise plans", icon: "", tagline: "Admin controls over project sharing inside the org.", description: "Enterprise admin policy layer governing how projects can be shared across the workspace and externally. Pairs with Publishing Controls and Audit Logs for a full governance stack.", capabilities: ["Share policy enforcement","External-share restrictions","Workspace-scoped sharing"], useCases: ["Data-sensitive industries","IP protection","Limiting external collaborator exposure"], source: "https://lovable.dev/pricing" },
  { id: "lovable-experts", name: "Lovable Experts Marketplace", category: "Community", status: "GA" as const, releaseDate: "2025-03-01", pricing: "Free to browse; partners set their own rates", icon: "", tagline: "Hire vetted Lovable agencies to build for you.", description: "Live partner marketplace connecting Lovable users with independent expert agencies for paid build engagements. Partners publish hourly rates, minimum project budgets, hire counts, and portfolios. Partners are independent entities, not Lovable employees.", capabilities: ["Partner directory","Hourly rates visible","Hire counts and portfolios","Direct partner contact"], useCases: ["Hire someone to build for you","Agency engagements","Larger custom builds"], source: "https://lovable.dev/experts" },
  { id: "lovable-affiliate-program", name: "Lovable Affiliate Program", category: "Community", status: "Beta" as const, releaseDate: "2026-04-01", pricing: "Free to join", icon: "", tagline: "Refer Lovable, earn rewards (launching soon).", description: "Referral-based affiliate program for content creators and community members. Interested parties email affiliates@lovable.dev. Not yet fully open — listed as Beta until full launch.", capabilities: ["Unique referral links","Real-time tracking","Payout on conversion"], useCases: ["Content creators","Community builders","Newsletter operators"], source: "https://lovable.dev/affiliates" },
  { id: "community-templates-gallery", name: "Community Templates Gallery", category: "Community", status: "GA" as const, releaseDate: "2025-02-07", pricing: "Free", icon: "", tagline: "100+ production-ready community templates as starting points.", description: "Public gallery of 100+ user-created Lovable templates spanning portfolios, SaaS landing pages, ecommerce, dashboards, project management, resume builders, wedding sites, and more. Distinct from the in-product Design Templates Business-plan feature — this is the public community surface.", capabilities: ["Public template directory","Forkable starting points","Category browsing"], useCases: ["Project kickstart","Pattern reference","Showcasing your build"], source: "https://lovable.dev/templates" },
  { id: "lovable-launched-directory", name: "Lovable Launched Directory", category: "Community", status: "GA" as const, releaseDate: "2025-02-07", pricing: "Free", icon: "", tagline: "Public directory to launch and discover apps built on Lovable.", description: "App-discovery directory where Lovable builders publish their apps to a public catalog and browse what others have shipped. Launched February 2025 alongside Lovable's Product Hunt debut.", capabilities: ["Public app directory","App discovery","Social proof for builders"], useCases: ["Showcase your build","Browse community apps","Find templates and inspiration"], source: "https://www.producthunt.com/products/lovable" },
  { id: "discoverable-by-default", name: "Discoverable by Default", category: "Publishing", status: "GA" as const, releaseDate: "2026-05-13", pricing: "Free on all plans", icon: "", tagline: "Every published app ships ready to be found on Google and AI search.", description: "Lovable apps are now built to be read by search engines and AI crawlers (ChatGPT, Claude, Perplexity) from the moment you publish. New apps get discoverability baked in by default; existing apps get the upgrade automatically with no action needed. Closes the gap between shipping and getting found.", capabilities: ["Crawler-ready HTML on publish","Automatic upgrade for existing apps","Indexed by Google and AI search engines","No setup or config required"], useCases: ["Marketing sites and landing pages","Public SaaS products","Content and directory sites"], source: "https://lovable.dev/seo-aeo" },
  { id: "chat-with-seo-data", name: "Chat with Your SEO Data", category: "Editor", status: "GA" as const, releaseDate: "2026-05-13", pricing: "Standard build credits; Semrush data free until Aug 15, 2026", icon: "", tagline: "Semrush data built into the agent — ask, find, and ship in one chat.", description: "The Lovable agent now has Semrush data built in. Ask how your app performs in search, surface keyword opportunities, or have Lovable spin up optimized landing pages — all in the same chat where you build. Costs regular build credits; the underlying Semrush data is free through August 15, 2026.", capabilities: ["Live Semrush data inside the agent","Keyword and opportunity research in chat","Auto-generated optimized landing pages","Search performance Q&A"], useCases: ["Keyword research without leaving the editor","Spinning up SEO landing pages on demand","Validating organic positioning mid-build"], source: "https://lovable.dev/seo-aeo" },
  { id: "seo-review-dashboard", name: "SEO Review Dashboard", category: "Editor", status: "GA" as const, releaseDate: "2026-05-13", pricing: "Standard build credits", icon: "", tagline: "On-demand SEO and AEO reviews with one-click fixes.", description: "Run an SEO review on demand and get clear, prioritized recommendations across both search engines and AI answers, right in the SEO dashboard. Apply fixes in one click and let the agent handle the implementation. Costs regular build credits.", capabilities: ["On-demand SEO + AEO scans","Prioritized recommendations","One-click agent fixes","Covers Google and AI search"], useCases: ["Pre-launch SEO audit","Iterating on a published site","Fixing AEO blind spots quickly"], source: "https://lovable.dev/seo-aeo" },
];
