...
-- Keep every digest table private to the server. The service role is the only
-- database role used by the digest server functions; browsers never need
-- direct table access.
...
-- Remove the retired direct-client subscription path even when the earlier
-- hardening migration was not applied to a restored environment.
DROP POLICY IF EXISTS "Anon can subscribe (pending only)"
  ON public.digest_subscribers;

-- Lovable's scanner expects a conventional allow policy to describe the
-- trusted role. service_role already bypasses RLS in Supabase, but these
-- policies make that boundary explicit without changing runtime behavior.
DROP POLICY IF EXISTS "Service role can manage digest subscribers"
  ON public.digest_subscribers;
...
DROP POLICY IF EXISTS "Service role can manage digest subscribe attempts"
...
DROP POLICY IF EXISTS "Service role can manage digest send log"
...
DROP POLICY IF EXISTS "Service role can manage digest email log"
...
-- Retain explicit restrictive client denials so future grants cannot expose
-- rows accidentally. Recreating them also makes this migration self-healing.
